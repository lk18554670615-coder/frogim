    ...
    public static class ScreenRequestPermissionsFragment extends Fragment {
        ...
        public void requestStart(Activity activity, int requestCode) {
            if (android.os.Build.VERSION.SDK_INT < minAPILevel) {
                ...
                // Android 14's default consent flow can capture one app task.
                // A call launched from CallKit may leave more than one task in
                // recents, which makes that picker capable of sharing a stale
                // login task. This product shares the current display instead,
                // so request the default display explicitly on API 34+.
