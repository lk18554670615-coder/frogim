...
class Setting {
  ...
  int encode() {
    return receipt << 7 | topic << 3 | stream << 1;
  }
}
...
RecvPacket decodeRecv(PacketHeader header, ReadData reader) {
  ...
  if (WKIM.shared.options.protoVersion >= 2 &&
      WKIM.shared.options.protoVersion < 5 &&
      recv.setting.stream == 1) {
    // WuKongIMGoProto v1.2.3 wire order is flag, no, uint64 stream id.
    // The upstream Flutter model calls that final field streamSeq; retain the
    // public name while decoding the pinned wire value losslessly.
    recv.streamFlag = reader.readUint8().toInt();
    recv.streamNo = reader.readString();
    recv.streamSeq = reader.readUint64().toInt();
  }
