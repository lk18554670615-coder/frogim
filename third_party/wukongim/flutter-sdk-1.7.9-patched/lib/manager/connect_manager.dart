...
class WKConnectionManager {
  ...
  bool _isCurrentConnection(int generation) {
    return !isDisconnection && generation == _connectionGeneration;
  }
  ...
  _decodePacket(Uint8List data) {
    ...
    if (packet.header.packetType == PacketType.connack) {
      ...
      final eventPacket = packet as EventPacket;
      WKIM.shared.eventManager.notifyEventListeners(WKEvent(eventPacket));
  ...
  _sendConnectPacket(int generation) async {
    CryptoUtils.init();
