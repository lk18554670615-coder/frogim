...
func (x *API) createAdministrator(w http.ResponseWriter, r *http.Request) {
	var request struct {
		Email, DisplayName, RoleID, Password, Reason string
		Confirmed                                    bool
	}
	if decode(r, &request) != nil {
	...
	item, err := x.app.CreateAdminAccount(r.Context(), uid(r), request.Email, request.DisplayName, request.RoleID, request.Password)
	if err != nil {
		handleErr(w, err)
		return
	}
	x.app.RecordAdminAudit(uid(r), "administrator.create", "administrator", item.ID, "success", x.clientIP(r), map[string]any{"reason": strings.TrimSpace(request.Reason), "roleId": item.RoleID})
	write(w, http.StatusCreated, item)
}

func (x *API) updateAdministrator(w http.ResponseWriter, r *http.Request) {
	var request struct {
		Email, DisplayName, RoleID, Status *string
		Reason                             string
		Confirmed                          bool
	}
	if decode(r, &request) != nil || (request.Email == nil && request.DisplayName == nil && request.RoleID == nil && request.Status == nil) {
		writeError(w, http.StatusBadRequest, "INVALID_ARGUMENT", "at least one administrator field is required")
		return
	}
	item, err := x.app.UpdateAdminAccount(r.Context(), uid(r), r.PathValue("id"), request.Email, request.DisplayName, request.RoleID, request.Status)
	if err != nil {
		handleErr(w, err)
		return
	}
	x.app.RecordAdminAudit(uid(r), "administrator.update", "administrator", item.ID, "success", x.clientIP(r), map[string]any{"reason": strings.TrimSpace(request.Reason), "roleId": item.RoleID, "status": item.Status})
