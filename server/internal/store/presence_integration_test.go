...
func TestPostgresPresencePermissions(t *testing.T) {
	...
	// Ordinary users, including group owners and administrators, cannot inspect
	// another user's online state.
	check("member", "")
	check("owner", "group")
	check("admin", "group")
	exec(`UPDATE im_users SET is_internal_user=true WHERE id='member'`)
	check("member", "", "friend")
	check("member", "group", "owner", "admin", "member")
	exec(`UPDATE im_users SET is_internal_user=true WHERE id='owner'`)
	check("owner", "group", "owner", "admin", "member")
	exec(`UPDATE im_users SET is_internal_user=true WHERE id='admin'`)
	check("admin", "group", "owner", "admin", "member")
	check("outsider", "group")
	check("admin", "wrong-group")
	...
	check("member", "")
	exec(`UPDATE im_users SET banned=true WHERE id='owner'`)
	check("owner", "group")
	exec(`UPDATE im_users SET banned=false WHERE id='owner'`)
