...
describe('青蛙呱呱管理后台', () => {
  ...
  it('按业务域组织侧栏并自动展开当前页面分组', async () => {
    ...
    expect(within(navigation).getByRole('button', { name: '基础设施' })).toHaveAttribute('aria-expanded', 'true');
    expect(within(navigation).getByRole('button', { name: '用户与关系' })).toHaveAttribute('aria-expanded', 'false');
    ...
    await userEvent.click(within(navigation).getByRole('button', { name: '用户与关系' }));
    expect(within(navigation).getByRole('button', { name: '用户与关系' })).toHaveAttribute('aria-expanded', 'true');
  });

  it('顶部命令搜索只检索真实功能路由并完成导航', async () => {
    render(<App />);
    const user = userEvent.setup();
    const search = screen.getByRole('searchbox', { name: '搜索后台功能' });
    await user.type(search, '用户管理');
    await user.click(screen.getByRole('option', { name: /用户管理/ }));
  ...
  it('兼容服务端扁平 dashboard 响应', async () => {
    ...
    expect(screen.getByText('活跃群组')).toBeInTheDocument();
    expect(screen.getByText('5 个业务会话')).toBeInTheDocument();
    expect(screen.getByText('当前仅提供累计消息')).toBeInTheDocument();
  ...
  it('运行概览不会把服务端缺失的计数和趋势伪造成零', async () => {
    ...
    expect(screen.getByText('消息统计未上报')).toBeInTheDocument();
