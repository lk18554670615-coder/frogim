...
function UserDetailDialog({ user, onClose }: { user: UserRecord; onClose: () => void }) {
  ...
  return <DetailDialog title="用户详情" detail="账号资料、关系、黑名单与设备均来自服务端实时查询。" onClose={onClose}>
    ...
    {overview && <><div className="detail-identity"><UserAvatar user={currentUser} detail /><div><h3>{currentUser.nickname}</h3><p className="mono">{currentUser.id}</p></div><div className="detail-user-status"><Badge value={currentUser.status} /><Badge value={currentUser.online ? 'active' : 'neutral'} label={currentUser.online ? `在线 · ${currentUser.onlineConnections || 1} 个连接` : '离线'} /></div></div>
      <div className="detail-metrics"><div><span>设备</span><strong>{overview.deviceCount}</strong></div><div><span>好友</span><strong>{overview.friendCount}</strong></div><div><span>加入群组</span><strong>{overview.groupCount}</strong></div><div><span>呱呱号剩余修改</span><strong>{overview.handleChangesRemaining}</strong></div><div><span>个人邀请码</span><strong className="mono">{overview.invitation?.code || '未生成'}</strong><small>{overview.invitation ? `${overview.invitation.status === 'active' ? '正常' : overview.invitation.status === 'disabled' ? '已停用' : '已退休'} · 自主修改剩余 ${overview.invitation.selfChangesRemaining} 次` : '尚未生成'}</small></div><div><span>邀请来源</span><strong>{overview.invitation?.invitedBy?.nickname || '无邀请关系'}</strong><small>{overview.invitation?.invitedBy ? `${overview.invitation.registrationMethod === 'password' ? '密码注册' : '验证码开户'} · ${overview.invitation.boundAt}` : '账号创建后不可补填或转移'}</small></div></div>
...
function InvitationsPage() {
  ...
  return <><PageHeader title="邀请管理" description="管理账号注册邀请码和不可变的邀请关系；历史邀请码永久保留且不能再次分配。" />
