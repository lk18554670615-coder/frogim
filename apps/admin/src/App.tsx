...
const joinPolicyLabels: Record<string, string> = {
  invite: '仅成员邀请', manager_invite: '仅群主／管理员邀请', member_approval: '成员邀请需管理员验证', qr: '二维码加入', closed: '暂停加入',
};

type GroupSettingsDraft = {
  name: string; avatarMediaId: string; announcement: string; joinPolicy: string;
  allowMemberAddFriend: boolean; historyVisibleToNewMembers: boolean;
  memberMessageRateLimitPerMinute: 0 | 5 | 10 | 20; allMuted: boolean;
};

const groupSettingsDraft = (data: GroupOverview): GroupSettingsDraft => ({
  name: data.title, avatarMediaId: data.avatarMediaId, announcement: data.announcement, joinPolicy: data.joinPolicy,
  allowMemberAddFriend: data.allowMemberAddFriend, historyVisibleToNewMembers: data.historyVisibleToNewMembers,
  memberMessageRateLimitPerMinute: data.memberMessageRateLimitPerMinute, allMuted: Boolean(data.allMutedUntil),
});
...
function GroupSettingsPanel({ data, groupId, reload, onUpdated }: { data: GroupOverview; groupId: string; reload: () => Promise<unknown>; onUpdated: () => Promise<unknown> | void }) {
  const { api, can, notify } = useApi();
  const editable = can('groups.write') && !data.dissolvedAt;
  const [draft, setDraft] = useState<GroupSettingsDraft>(() => groupSettingsDraft(data));
  const [confirming, setConfirming] = useState(false), [reason, setReason] = useState('');
  const [avatarPreview, setAvatarPreview] = useState(data.avatarUrl), [avatarFile, setAvatarFile] = useState<File>();
  const [uploading, setUploading] = useState(false), [uploadError, setUploadError] = useState(''), [conflict, setConflict] = useState(false);
  const fileInput = useRef<HTMLInputElement>(null), objectURL = useRef('');
  useEffect(() => {
    setDraft(groupSettingsDraft(data)); setAvatarPreview(data.avatarUrl); setAvatarFile(undefined); setUploadError(''); setConflict(false);
    if (objectURL.current) { URL.revokeObjectURL(objectURL.current); objectURL.current = ''; }
  }, [data.updatedAt, data.avatarUrl]);
  useEffect(() => () => { if (objectURL.current) URL.revokeObjectURL(objectURL.current); }, []);
  const changes = useMemo(() => {
    const next: GroupSettingsChanges = {};
    if (draft.name.trim() !== data.title) next.name = draft.name.trim();
    if (draft.avatarMediaId !== data.avatarMediaId) next.avatarMediaId = draft.avatarMediaId;
    if (draft.announcement.trim() !== data.announcement) next.announcement = draft.announcement.trim();
    if (draft.joinPolicy !== data.joinPolicy) next.joinPolicy = draft.joinPolicy;
    if (draft.allowMemberAddFriend !== data.allowMemberAddFriend) next.allowMemberAddFriend = draft.allowMemberAddFriend;
    if (draft.historyVisibleToNewMembers !== data.historyVisibleToNewMembers) next.historyVisibleToNewMembers = draft.historyVisibleToNewMembers;
    if (draft.memberMessageRateLimitPerMinute !== data.memberMessageRateLimitPerMinute) next.memberMessageRateLimitPerMinute = draft.memberMessageRateLimitPerMinute;
    if (draft.allMuted !== Boolean(data.allMutedUntil)) next.allMuted = draft.allMuted;
    return next;
  }, [data, draft]);
  const changedKeys = Object.keys(changes);
  const labels: Record<string, string> = { name: '群名称', avatarMediaId: '群头像', announcement: '群公告', joinPolicy: '入群方式', allowMemberAddFriend: '成员互加好友', historyVisibleToNewMembers: '入群前历史', memberMessageRateLimitPerMinute: '成员发言频率', allMuted: '永久全员禁言' };
  const uploadAvatar = async (file = avatarFile) => {
    if (!file) return;
    if (!file.type.startsWith('image/')) { setUploadError('请选择图片文件'); return; }
    setUploading(true); setUploadError(''); setAvatarFile(file);
    if (objectURL.current) URL.revokeObjectURL(objectURL.current);
    objectURL.current = URL.createObjectURL(file); setAvatarPreview(objectURL.current);
    try {
      const prepared = await api.prepareGroupAvatar(groupId, { mime: file.type, fileName: file.name, size: file.size });
      ...
      const completed = await api.completeGroupAvatar(groupId, prepared.mediaId, await fileSHA256(file));
      setDraft(current => ({ ...current, avatarMediaId: completed.mediaId }));
    } catch (cause) { setUploadError(errorMessage(cause)); } finally { setUploading(false); }
  };
  const cancelAvatar = () => {
    if (objectURL.current) { URL.revokeObjectURL(objectURL.current); objectURL.current = ''; }
    setAvatarFile(undefined); setUploadError(''); setAvatarPreview(data.avatarUrl); setDraft(current => ({ ...current, avatarMediaId: data.avatarMediaId }));
  };
  const save = async () => {
    if (!reason.trim() || !changedKeys.length) return;
    try {
      await api.updateGroupSettings(groupId, data.updatedAt, changes, reason.trim());
    } catch (cause) {
      if (cause instanceof ApiError && cause.code === 'GROUP_SETTINGS_CHANGED') setConflict(true);
      throw cause;
    }
    notify('群设置已原子保存并通知群成员'); setReason(''); setConflict(false); await reload(); await onUpdated();
  };
  const resetDraft = () => { cancelAvatar(); setDraft(groupSettingsDraft(data)); setConflict(false); };
  return <section className="detail-section user-detail-section group-settings-panel">
    <div className="detail-section-heading"><div><h3>群设置</h3><p>{editable ? '修改会集中确认并原子保存；任何字段失败都不会部分生效。' : '当前账号仅可查看，或群组已解散，不能编辑。'}</p></div><div className="row-actions wrap"><button className="button secondary compact" disabled={!changedKeys.length} onClick={resetDraft}>放弃草稿</button><button className="button primary compact" disabled={!editable || !changedKeys.length || uploading || !draft.name.trim()} onClick={() => { setReason(''); setConfirming(true); }}>保存设置</button></div></div>
    {conflict && <div className="inline-notice danger" role="alert"><AlertTriangle size={15}/>群设置已被其他人修改。当前草稿仍保留，请先重新加载再核对。<button className="button secondary compact" onClick={() => void reload()}>重新加载</button></div>}
    <div className="group-settings-sections">
      <div className="settings-section"><div className="settings-title"><CircleUserRound size={18}/><div><h2>基本资料</h2><p>名称、头像和公告会同步到群成员客户端。</p></div></div><div className="group-avatar-editor"><span>{avatarPreview ? <img src={avatarPreview} alt="群头像预览"/> : <Users size={24}/>}</span><div className="row-actions wrap"><input ref={fileInput} type="file" accept="image/*" hidden onChange={event => { const file = event.target.files?.[0]; if (file) void uploadAvatar(file); event.currentTarget.value = ''; }}/>{editable && <><button className="button secondary compact" disabled={uploading} onClick={() => fileInput.current?.click()}>{uploading ? '上传中…' : '选择头像'}</button>{draft.avatarMediaId && <button className="button secondary compact danger-text" disabled={uploading} onClick={() => { cancelAvatar(); setDraft(current => ({ ...current, avatarMediaId: '' })); setAvatarPreview(''); }}>移除头像</button>}</>}{uploadError && <><small className="danger-text">{uploadError}</small><button className="button secondary compact" disabled={uploading} onClick={() => void uploadAvatar()}>重试</button><button className="button secondary compact" onClick={cancelAvatar}>取消</button></>}</div></div><div className="form-grid"><label className="field-label">群名称<input value={draft.name} maxLength={80} disabled={!editable} onChange={event => setDraft(current => ({ ...current, name: event.target.value }))}/></label><label className="field-label detail-list-wide">群公告<textarea value={draft.announcement} maxLength={5000} disabled={!editable} onChange={event => setDraft(current => ({ ...current, announcement: event.target.value }))}/></label></div></div>
      <div className="settings-section"><div className="settings-title"><Users size={18}/><div><h2>入群与成员</h2><p>入群方式变化会使所有待审核申请失效。</p></div></div><div className="form-grid"><label className="field-label">入群方式<select value={draft.joinPolicy} disabled={!editable} onChange={event => setDraft(current => ({ ...current, joinPolicy: event.target.value }))}>{Object.entries(joinPolicyLabels).map(([value,label]) => <option value={value} key={value}>{label}</option>)}</select></label></div><label className={`toggle-row ${!editable?'disabled':''}`}><div><strong>允许同群成员添加好友</strong><span>成员可从群资料发起好友申请。</span></div><input type="checkbox" role="switch" aria-label="允许同群成员添加好友" checked={draft.allowMemberAddFriend} disabled={!editable} onChange={event => setDraft(current => ({ ...current, allowMemberAddFriend: event.target.checked }))}/><span className="toggle-control"/></label><label className={`toggle-row ${!editable?'disabled':''}`}><div><strong>新成员可查看入群前历史</strong><span>开启后当前成员可查看全部群历史，关闭后按本次入群边界限制。</span></div><input type="checkbox" role="switch" aria-label="新成员可查看入群前历史" checked={draft.historyVisibleToNewMembers} disabled={!editable} onChange={event => setDraft(current => ({ ...current, historyVisibleToNewMembers: event.target.checked }))}/><span className="toggle-control"/></label></div>
      <div className="settings-section"><div className="settings-title"><MessageSquareText size={18}/><div><h2>消息策略</h2><p>发言频率仅限制普通成员；群主和管理员不受影响。</p></div></div><div className="form-grid"><label className="field-label">普通成员每分钟发言频率<select value={draft.memberMessageRateLimitPerMinute} disabled={!editable} onChange={event => setDraft(current => ({ ...current, memberMessageRateLimitPerMinute: Number(event.target.value) as 0|5|10|20 }))}><option value={0}>不限</option><option value={5}>最多 5 条</option><option value={10}>最多 10 条</option><option value={20}>最多 20 条</option></select></label></div><label className={`toggle-row ${!editable?'disabled':''}`}><div><strong>永久全员禁言</strong><span>开启后普通成员无法发言，群主和管理员仍可发送消息。</span></div><input type="checkbox" role="switch" aria-label="永久全员禁言" checked={draft.allMuted} disabled={!editable} onChange={event => setDraft(current => ({ ...current, allMuted: event.target.checked }))}/><span className="toggle-control"/></label></div>
    </div>
    <ConfirmDialog open={confirming} title="保存群设置" detail={`将一次保存 ${changedKeys.length} 项变更：${changedKeys.map(key => labels[key] ?? key).join('、')}。`} confirmLabel="确认保存" danger={changes.allMuted === true || changes.historyVisibleToNewMembers === true} confirmDisabled={!reason.trim()} onClose={() => setConfirming(false)} onConfirm={save}><div className="inline-notice"><AlertTriangle size={15}/><span>{changes.joinPolicy ? '入群方式变化会使待审核申请失效。' : ''}{changes.historyVisibleToNewMembers === true ? ' 开启历史后当前成员可查看全部群历史。' : ''}{changes.allMuted === true ? ' 开启全员禁言后普通成员将立即无法发言。' : ''}</span></div><label className="field-label">操作理由<textarea value={reason} maxLength={500} onChange={event => setReason(event.target.value)} placeholder="填写运营工单或修改依据" required/></label></ConfirmDialog>
  </section>;
}
...
function GroupDetailDialog({ group, onClose, onUpdated }: { group: GroupRecord; onClose: () => void; onUpdated: () => Promise<unknown> | void }) {
  const { api, mode, notify, can } = useApi();
  const [section, setSection] = useState<'profile' | 'settings' | 'members' | 'messages' | 'blacklist'>('profile');
  ...
  const [governance, setGovernance] = useState<'ban' | 'unban'>(); const [governanceReason, setGovernanceReason] = useState('');
  ...
  const memberUser = (member: GroupMemberRecord): UserRecord => ({ id: member.userId, nickname: member.groupNickname || member.name, phone: member.phone || '未提供', handle: member.handle, avatar: (member.groupNickname || member.name || '?').slice(0, 1), avatarUrl: member.avatarUrl, status: 'active', isInternalUser: false, online: false, onlineConnections: 0, registeredAt: member.joinedAt, lastSeen: '暂无', deviceCount: 0, messageCount: 0, remark: '', tags: [], gender: 'unspecified', handleChangeCount: 0 });
  ...
  const executeGovernance = async () => { if (!governance || !governanceReason.trim()) throw new Error('请输入处置理由'); await api.setGroupBan(group.id, governance === 'ban', governanceReason.trim()); notify(governance === 'ban' ? '群组已封禁' : '群组已解封'); setGovernance(undefined); setGovernanceReason(''); await overview.reload(); await onUpdated(); };
  ...
  return <><DetailDialog title="群组详情" detail="群资料、成员、WuKongIM 历史消息与黑名单均来自服务端实时数据。" onClose={onClose}>
    ...
    {data && <><div className="detail-identity"><span className="group-detail-icon">{data.avatarUrl?<img src={data.avatarUrl} alt=""/>:<Users size={20} />}</span><div><h3>{data.title}</h3><p className="mono">{data.id}</p></div><Badge value={data.dissolvedAt ? 'dissolved' : data.banned ? 'banned' : data.allMutedUntil ? 'muted' : 'active'} /></div>
      ...
      <div className="tabs detail-tabs" role="tablist" onKeyDown={tabListKeyDown}>{([['profile','概览'],['settings','群设置'],['members',`群成员 ${data.memberCount}`],['messages',`聊天记录 ${data.messageCount}`],['blacklist',`黑名单 ${blacklist.data?.length ?? 0}`]] as const).map(([value,label])=><button type="button" role="tab" aria-selected={section===value} className={section===value?'active':''} key={value} onClick={()=>setSection(value)}>{label}</button>)}</div>
      {section==='profile' && <section className="detail-section user-detail-section"><div className="detail-section-heading"><div><h3>群组概览与治理</h3><p>设置项在“群设置”集中修改；封禁可恢复，解散仍在列表页执行。</p></div><div className="row-actions wrap"><button className="button secondary compact" disabled={!can('groups.write')||Boolean(data.dissolvedAt)} onClick={()=>{setProxyOpen(true);setProxySender(undefined);setProxyContent('');setProxyReason('');}}>以群主发消息</button><button className="button secondary compact danger-text" disabled={!can('groups.write')||Boolean(data.dissolvedAt)} onClick={()=>{setGovernance(data.banned?'unban':'ban');setGovernanceReason('');}}>{data.banned?'解封群组':'封禁群组'}</button></div></div><dl className="detail-list"><div className="detail-list-wide"><dt>群主</dt><dd><UserIdentity user={data.owner} fallbackId={data.ownerId}/></dd></div><div><dt>加入方式</dt><dd>{joinPolicyLabels[data.joinPolicy]??data.joinPolicy}<small>策略版本 {data.joinPolicyVersion}</small></dd></div><div><dt>成员互加好友</dt><dd>{data.allowMemberAddFriend?'允许':'不允许'}</dd></div><div><dt>入群前历史</dt><dd>{data.historyVisibleToNewMembers?'全部可见':'按入群边界'}<small>策略版本 {data.historyPolicyVersion}</small></dd></div><div><dt>成员发言频率</dt><dd>{data.memberMessageRateLimitPerMinute ? `每分钟 ${data.memberMessageRateLimitPerMinute} 条` : '不限'}<small>策略版本 {data.messageRateLimitVersion}</small></dd></div><div><dt>永久全员禁言</dt><dd>{data.allMutedUntil?'已开启':'已关闭'}</dd></div><div className="detail-list-wide"><dt>群公告</dt><dd>{data.announcement||'尚未设置群公告'}</dd></div>{data.banned&&<div className="detail-list-wide"><dt>封禁依据</dt><dd>{data.banReason||'未记录'}{data.bannedAt&&<small>{dateTimeLabel(data.bannedAt)}</small>}</dd></div>}</dl></section>}
      {section==='settings' && <GroupSettingsPanel data={data} groupId={group.id} reload={overview.reload} onUpdated={onUpdated}/>}
    </>}
  ...
  <ConfirmDialog open={Boolean(governance)} title={governance==='ban'?'封禁群组':'解封群组'} detail={governance==='ban'?'保留成员和历史消息，但频道将停止通信。':'操作将立即同步到 WuKongIM。'} confirmLabel="确认操作" danger={governance==='ban'} confirmDisabled={!governanceReason.trim()} onClose={()=>setGovernance(undefined)} onConfirm={executeGovernance}><label className="field-label">操作理由<textarea value={governanceReason} maxLength={500} onChange={event=>setGovernanceReason(event.target.value)} required/></label></ConfirmDialog>
...
function CompactPermissionSwitch({ checked, disabled, onChange }: { checked: boolean; disabled: boolean; onChange: (checked: boolean) => void }) {
  const label = checked ? '内部' : '普通';
  return <label className={`compact-permission-switch${disabled ? ' disabled' : ''}`} title={disabled ? `用户类型：${label}用户（当前管理员无修改权限）` : `用户类型：${label}用户`}>
    <input type="checkbox" role="switch" aria-label="内部用户" checked={checked} disabled={disabled} onChange={event => onChange(event.target.checked)} />
...
function UserManagementCard({ user, canWrite, onInternalStatusChange, onDetail, onRecords, onMessage, onDisposition,onIP }: { user: UserRecord; canWrite: boolean; onInternalStatusChange: (internal:boolean) => void; onDetail: () => void; onRecords: () => void; onMessage: () => void; onDisposition: () => void;onIP:(ip:string)=>void }) {
  const {notify}=useApi();
  return <article className="user-management-card">
    ...
    <dl className="user-card-facts">
      ...
      <div><dt>内部用户</dt><dd><CompactPermissionSwitch checked={user.isInternalUser} disabled={!canWrite} onChange={onInternalStatusChange} /></dd></div>
...
function UsersPage({ scope = 'all' }: { scope?: 'all' | 'banned' }) {
  ...
  const [internalUserFilter,setInternalUserFilter]=useState('');
  const [internalStatusChange,setInternalStatusChange]=useState<{user:UserRecord;internal:boolean}>();const [internalStatusReason,setInternalStatusReason]=useState('');
  ...
  useEffect(() => { setPage(1); setCursors({ 1: '' }); }, [deferredQuery, status, pageSize,ip,ipSource,internalUserFilter]);
  const state = useResource(() => api.getUsers(deferredQuery, status, page, pageSize, cursors[page] ?? '',ip,ipSource,internalUserFilter), [api, mode, deferredQuery, status, page, pageSize, cursors,ip,ipSource,internalUserFilter]);
...
function GroupsPage({ scope = 'normal' }: { scope?: 'normal' | 'banned' }) {
  ...
  return <><PageHeader title={scope==='banned'?'封禁群列表':'群组列表'} description={scope==='banned'?'仅显示已封禁且未解散的群，可查看历史并恢复通信。':'检查群资料、群主、成员、消息、举报与治理状态。'} /><Toolbar query={query} setQuery={setQuery} placeholder="搜索群名称、群 ID、群主或手机号">{scope==='normal'&&<select className="select-control" aria-label="群组状态" value={status} onChange={e=>setStatus(e.target.value)}><option value="">全部状态</option><option value="active">正常</option><option value="muted">全员禁言</option><option value="dissolved">已解散</option></select>}</Toolbar>
    ...
    {detailGroup && <GroupDetailDialog group={detailGroup} onClose={() => setDetailGroup(undefined)} onUpdated={state.reload} />}
