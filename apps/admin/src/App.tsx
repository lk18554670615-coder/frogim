...
function GroupsPage({ scope = 'normal' }: { scope?: 'normal' | 'banned' }) {
  ...
  return <><PageHeader title={scope==='banned'?'封禁群列表':'群组列表'} description={scope==='banned'?'仅显示已封禁且未解散的群，可查看历史并恢复通信。':'检查群资料、群主、成员、消息、举报与治理状态。'} /><Toolbar query={query} setQuery={setQuery} placeholder="搜索群名称、群 ID、群主或手机号">{scope==='normal'&&<select className="select-control" aria-label="群组状态" value={status} onChange={e=>setStatus(e.target.value)}><option value="">全部状态</option><option value="active">正常</option><option value="muted">全员禁言</option><option value="dissolved">已解散</option></select>}</Toolbar>
    <DataPanel loading={state.loading} error={state.error} retry={state.reload} empty={!state.data?.items.length} emptyTitle={scope==='banned'?'当前没有封禁群组':'没有匹配的群组'} emptyDetail="请更换群名称、群 ID、群主昵称或手机号后重试。"><div className="table-wrap"><table><thead><tr><th>群组</th><th>状态</th><th>群主</th><th>成员</th><th>累计消息</th><th>举报</th><th>创建日期</th><th><span className="sr-only">操作</span></th></tr></thead><tbody>{state.data?.items.map((group) => <tr key={group.id}><td><div className="group-name"><span>{group.avatarUrl?<img src={group.avatarUrl} alt=""/>:<Users size={17} />}</span><div><strong>{group.name}</strong><small className="mono">{group.id}</small></div></div></td><td><Badge value={group.status} />{group.banReason&&<small>{group.banReason}</small>}</td><td><UserIdentity user={group.owner} fallbackId={group.ownerId}/></td><td>{group.memberCount}</td><td>{group.messageCount.toLocaleString()}</td><td>{group.reportCount}</td><td>{group.createdAt}</td><td><div className="row-actions wrap"><button className="row-action" onClick={() => setDetailGroup(group)}>查看详情</button>{scope==='normal'&&group.status!=='dissolved'&&<button className="row-action" disabled={!can('groups.write')} onClick={()=>{setSendGroup(group);setSendContent('');setSendReason('');}}>发消息</button>}<button className={`row-action ${group.banned?'':'danger-text'}`} disabled={group.status==='dissolved'||!can('groups.write')} onClick={()=>{setBanGroup(group);setBanReason('');}}>{group.banned?'解封':'封禁'}</button>{scope==='normal'&&<button className="row-action danger-text" disabled={group.status === 'dissolved' || !can('groups.write')} title={!can('groups.write') ? '当前角色没有群组处置权限' : undefined} onClick={() => openDisband(group)}>解散群组</button>}</div></td></tr>)}</tbody></table></div><Pagination data={state.data} onPage={paginate} /></DataPanel>
