    ...
    <div className="workspace">
      <div className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <div className="topbar-user"><span className="admin-avatar">{[...session.displayName][0]}</span><span><strong>{session.displayName}</strong><small>{session.roleName || roleLabels[session.roleId] || session.roleId}</small></span></div>
        </div>
      </div>
...
function UserManagementCard({ user, canWrite, onDetail, onRecords, onMessage, onDisposition }: { user: UserRecord; canWrite: boolean; onDetail: () => void; onRecords: () => void; onMessage: () => void; onDisposition: () => void }) {
  ...
  return <article className="user-management-card">
    <button type="button" className="user-card-identity" onClick={onDetail} aria-label={`查看 ${user.nickname} 的完整资料`}>
      ...
      <span><strong>{user.nickname}</strong><small className="mono">{user.id}</small><small>{user.phone}</small></span>
      <Badge value={user.status} />
    </button>
    <dl className="user-card-facts">
      <div><dt>呱呱号</dt><dd className="mono">{user.handle}</dd><small>{user.gender === 'male' ? '男' : user.gender === 'female' ? '女' : '未设置性别'} · 已修改 {user.handleChangeCount}/2 次</small></div>
      ...
      <div><dt>最近设备</dt><dd>{deviceTitle}</dd><small>{user.latestDevice ? `${user.latestDevice.platform} · ${user.latestDevice.osVersion || '系统未知'} · App ${user.latestDevice.appVersion}` : '等待新版客户端上报'}</small></div>
...
function LoginPage({ onLogin, sessionNotice = '' }: { onLogin: (email: string, password: string) => Promise<void>; sessionNotice?: string }) {
  ...
  return <main className="login-screen">
    <section className="login-form-panel" aria-labelledby="login-title"><div className="login-card">
      ...
      <div className="login-heading"><h1 id="login-title">欢迎回来</h1><p>请输入管理员邮箱和密码</p></div>
      ...
      <form onSubmit={(event) => void submit(event)} noValidate>
        <div className="field-label"><label htmlFor="admin-email">管理员邮箱</label><input id="admin-email" type="email" value={email} onBlur={() => { if (emailDirty) setEmailTouched(true); }} onChange={(event) => { setEmail(event.target.value); setEmailDirty(true); if (error) setError(''); }} autoComplete="username" required autoFocus aria-invalid={Boolean(emailError)} aria-describedby="admin-email-help" /><small id="admin-email-help" className={emailError ? 'field-error' : 'field-hint'}>{emailError || '使用管理员分配的邮箱地址'}</small></div>
        ...
        {error && <div className="inline-notice danger" role="alert"><AlertTriangle size={15} />{error}</div>}
        <button className="button primary full login-submit" type="submit" disabled={submitting || !normalizedEmail || emailInvalid || !password}><LogIn size={17} />{submitting ? '正在验证…' : '登录控制台'}</button>
      </form>
    </div></section>
