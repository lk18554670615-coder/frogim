...
function UserManagementCard({ user, canWrite, onDetail, onRecords, onMessage, onDisposition }: { user: UserRecord; canWrite: boolean; onDetail: () => void; onRecords: () => void; onMessage: () => void; onDisposition: () => void }) {
...
function UsersPage({ scope = 'all' }: { scope?: 'all' | 'banned' }) {
  ...
  useEffect(() => { setPage(1); setCursors({ 1: '' }); }, [deferredQuery, status, pageSize]);
  const state = useResource(() => api.getUsers(deferredQuery, status, page, pageSize, cursors[page] ?? ''), [api, mode, deferredQuery, status, page, pageSize, cursors]);
  ...
  return <><PageHeader title={scope === 'banned' ? '封禁用户' : '用户管理'} description={scope === 'banned' ? '集中查看当前仍处于封禁状态的账号，可进入完整资料核对后解除封禁。' : '查询真实账号资料，查看好友、黑名单和设备，并执行消息通知与账号治理。'} actions={<button type="button" className="button primary" disabled={!can('users.write')} onClick={() => { window.history.pushState({}, '', '/users/new'); window.dispatchEvent(new PopStateEvent('popstate')); }}><Plus size={16} />新增用户</button>} />
