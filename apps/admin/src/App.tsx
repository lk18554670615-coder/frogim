...
function UserManagementCard({ user, canWrite, selectedForIP, onSelectedForIP, onDetail, onRecords, onMessage, onDisposition,onIP }: { user: UserRecord; canWrite: boolean; selectedForIP: boolean; onSelectedForIP: (selected:boolean) => void; onDetail: () => void; onRecords: () => void; onMessage: () => void; onDisposition: () => void;onIP:(ip:string)=>void }) {
  const {notify}=useApi();
  return <article className="user-management-card">
    ...
    <dl className="user-card-facts">
      ...
      <div><dt>好友 IP 查看</dt><dd><Badge value={user.canViewFriendLoginIp ? 'active' : 'neutral'} label={user.canViewFriendLoginIp ? '已授权' : '未授权'} /></dd></div>
