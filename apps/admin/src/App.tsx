...
function InviteSourceEditor({ user, invitation, reload }: { user: UserRecord; invitation: UserOverview['invitation']; reload: () => Promise<unknown> }) {
  const { api, can, notify } = useApi();
  const [editing, setEditing] = useState(false);
  const [code, setCode] = useState(''), [reason, setReason] = useState('');
  const [version, setVersion] = useState(0);
  const [preview, setPreview] = useState<{ code: string; reason: string; version: number; inviter: UserRecord }>();
  const bound = Boolean(invitation?.invitedBy);
  const checkCode = async () => {
    const normalized = code.trim().toUpperCase();
    const page = await api.getInviteCodes(normalized, 'active', 1, 100);
    const target = page.items.find(item => item.code.toUpperCase() === normalized);
    if (!target || target.user.status === 'banned') throw new Error('邀请码无效、已停用或邀请人账号不可用');
    if (target.userId === user.id) throw new Error('不能绑定用户自己的邀请码');
    setPreview({code: normalized, reason: reason.trim(), version, inviter: target.user});
  };
  return <section className="detail-section">
    <div className="detail-section-heading"><div><h3>邀请来源管理</h3><p>修改注册邀请来源或为未绑定用户补绑；不修改个人邀请码，不消耗自主修改次数。</p></div>
      <div className="row-actions wrap"><button className="button secondary compact" onClick={() => void reload()}>刷新邀请来源</button>
      {can('users.write') && <button className="button secondary compact" onClick={() => {setCode(''); setReason(''); setVersion(invitation?.bindingVersion ?? 0); setEditing(true);}}>{bound ? '修改邀请来源' : '补绑邀请来源'}</button>}</div>
    </div>
    {bound && <p>绑定邀请码：<span className="mono">{invitation?.boundCode || '未记录'}</span> · {invitation?.bindingSource === 'admin' ? '后台维护' : '注册绑定'}{invitation?.bindingUpdatedAt && ` · 最近变更 ${invitation.bindingUpdatedAt}`}</p>}
    <ConfirmDialog open={editing} title={bound ? '修改邀请来源' : '补绑邀请来源'} detail={`目标用户：${user.nickname}（${user.id}）。当前邀请人：${invitation?.invitedBy?.nickname || '未绑定'}。先核对新邀请码，再确认保存。`} confirmLabel="核对邀请码" confirmDisabled={!/^[A-Z0-9][A-Z0-9_-]{4,18}[A-Z0-9]$/i.test(code.trim()) || !reason.trim()} onClose={() => setEditing(false)} onConfirm={checkCode}>
      <label className="field-label">新的邀请来源码<input value={code} maxLength={24} onChange={event => setCode(event.target.value)} placeholder="输入邀请人的有效邀请码" autoComplete="off" required /></label>
      <label className="field-label">改绑操作理由<textarea value={reason} maxLength={500} onChange={event => setReason(event.target.value)} placeholder="填写用户申请、运营工单或纠错依据" required /></label>
    </ConfirmDialog>
    <ConfirmDialog open={Boolean(preview)} title="确认邀请来源变更" detail={`将 ${user.nickname} 的邀请来源从「${invitation?.invitedBy?.nickname || '未绑定'}」改为以下用户。原关系将被替换，变更前后会写入审计。`} confirmLabel="确认保存邀请来源" onClose={() => setPreview(undefined)} onConfirm={async () => {
      if (!preview) return;
      await api.setUserInviteRelation(user.id, preview.code, preview.reason, preview.version);
      setPreview(undefined);
      notify('邀请来源已更新并记录审计');
      await reload();
    }}>
      {preview && <><UserIdentity user={preview.inviter} fallbackId={preview.inviter.id}/><p>新邀请码：<strong className="mono">{preview.code}</strong></p><p>操作理由：{preview.reason}</p></>}
    </ConfirmDialog>
  </section>;
}
...
function useCompactNavigation() {
  return useMediaQuery('(max-width: 1120px)');
}
...
const statusMap: Record<string, { label: string; tone: StatusTone }> = {
  ...
  hidden: { label: '已隐藏', tone: 'warning' }, deleted: { label: '已删除', tone: 'danger' }, disabled: { label: '已停用', tone: 'neutral' }, retired: { label: '已退休', tone: 'neutral' },
...
function UserAvatar({ user, detail = false }: { user: UserRecord; detail?: boolean }) {
  ...
  return <span className={`avatar ${detail ? 'detail-avatar' : ''}`}>
    <span className="avatar-fallback" style={{ visibility: showImage ? 'hidden' : 'visible' }}>{user.avatar}</span>{showImage && <img src={user.avatarUrl} alt="" onError={() => setFailedAvatarUrl(user.avatarUrl ?? '')} />}
  </span>;
}
...
function messageBodySummary(message: Pick<AdminDirectMessageRecord, 'body' | 'type' | 'deletedForEveryoneAt' | 'deletedForEveryoneBy'>): string {
  if(message.deletedForEveryoneAt) return `已全端删除 · 操作者 ${message.deletedForEveryoneBy || '未知'} · ${dateTimeLabel(message.deletedForEveryoneAt)}\n审核原文：${messageBodySummary({...message,deletedForEveryoneAt:undefined})}`;
...
function FriendConversationDialog({ owner, friend, onClose }: { owner: UserRecord; friend: UserRecord; onClose: () => void }) {
  ...
  return <><DetailDialog title={`${owner.nickname} 与 ${friend.nickname} 的聊天记录`} detail="正文从 WuKongIM 历史接口实时读取；查看和管理员撤回都会写入审计。" onClose={onClose}>
    ...
    {!!items.length && <div className="conversation-history">{items.map((message) => { const mediaURL = messageMediaURL(message); const unavailable = Boolean(message.deletedForEveryoneAt || message.recalledAt || message.expiredAt || message.deleted || message.adminRecall); return <article className="conversation-message" key={message.id}><UserAvatar user={message.sender ?? (message.senderId === owner.id ? owner : friend)} /><div className="conversation-message-main"><header><strong>{message.sender?.nickname || message.senderId}</strong><span>序号 {message.conversationSeq} · {message.createdAt}</span></header><div className={`conversation-message-body ${unavailable ? 'muted' : ''}`}>{message.deletedForEveryoneAt ? messageBodySummary(message) : message.adminRecall || message.deleted ? '该消息已被管理员撤回' : message.recalledAt ? '该消息已撤回' : message.expiredAt ? '该消息已过期' : messageBodySummary(message)}</div>{mediaURL && (!unavailable || message.deletedForEveryoneAt) && <a className="conversation-attachment" href={mediaURL} target="_blank" rel="noreferrer">查看附件</a>}{message.editedAt && !unavailable && <small>已编辑 · 版本 {message.editVersion}</small>}{message.adminRecall && <small>处置人：{message.moderatedBy || '管理员'} · {message.moderationReason || '未提供理由'}</small>}</div><button type="button" className="button secondary compact danger-text" disabled={unavailable} title={unavailable ? '已全端删除、撤回或过期的消息不能再次处置' : '管理员全端撤回'} onClick={() => { setReason(''); setRecalling(message); }}>撤回</button></article>; })}</div>}
...
function IPAccountsDialog({ip,onClose}:{ip:string;onClose:()=>void}) {
  const {api,notify}=useApi();const [currentIP,setCurrentIP]=useState(ip);
...
function UserDetailDialog({ user, onClose }: { user: UserRecord; onClose: () => void }) {
  ...
  const [deletionPermission, setDeletionPermission] = useState<boolean>();
  const [permissionReason, setPermissionReason] = useState('');
  const [friendIPPermission, setFriendIPPermission] = useState<boolean>();
  const [friendIPPermissionReason, setFriendIPPermissionReason] = useState('');
  ...
  return <DetailDialog title="用户详情" detail="账号资料、关系、黑名单与设备均来自服务端实时查询。" onClose={onClose}>
    ...
    {overview && <><div className="detail-identity"><UserAvatar user={currentUser} detail /><div><h3>{currentUser.nickname}</h3><p className="mono">{currentUser.id}</p></div><div className="detail-user-status"><Badge value={currentUser.status} /><Badge value={currentUser.online ? 'active' : 'neutral'} label={currentUser.online ? `在线 · ${currentUser.onlineConnections || 1} 个连接` : '离线'} /></div></div>
      <div className="detail-metrics"><div><span>设备</span><strong>{overview.deviceCount}</strong></div><div><span>好友</span><strong>{overview.friendCount}</strong></div><div><span>加入群组</span><strong>{overview.groupCount}</strong></div><div><span>呱呱号剩余修改</span><strong>{overview.handleChangesRemaining}</strong></div><div><span>个人邀请码</span><strong className="mono">{overview.invitation?.code || '未生成'}</strong><small>{overview.invitation ? `${overview.invitation.status === 'active' ? '正常' : overview.invitation.status === 'disabled' ? '已停用' : '已退休'} · 自主修改剩余 ${overview.invitation.selfChangesRemaining} 次` : '尚未生成'}</small></div><div><span>邀请来源</span><strong>{overview.invitation?.invitedBy?.nickname || '无邀请关系'}</strong><small>{overview.invitation?.invitedBy ? `${inviteBindingMethodLabel(overview.invitation.registrationMethod)} · ${overview.invitation.boundAt}` : '可由具有用户管理权限的管理员补绑'}</small></div></div>
      ...
      {section==='profile'&&<section className="detail-section message-permission-section"><div className="detail-section-heading"><div><h3>消息操作权限</h3><p>按用户单独授权；服务端会同时校验会话成员身份和群角色。</p></div></div><div className="message-permission-card"><Toggle label="允许全端删除消息" description={currentUser.canDeleteMessagesForEveryone===true?'已授权：单聊可删除双方消息；群聊还需是群主或管理员。':'未授权：用户只能使用本机删除和现有撤回功能。'} checked={currentUser.canDeleteMessagesForEveryone===true} disabled={!can('users.write')} onChange={allowed=>{setDeletionPermission(allowed);setPermissionReason('');}}/><div className="message-permission-meta"><span>不受撤回时限限制</span><span>删除后不显示占位</span><span>操作全程记录审计</span></div></div>{!can('users.write')&&<p className="message-permission-readonly">当前管理员没有用户写权限，仅可查看授权状态。</p>}</section>}
      {section==='profile'&&<section className="detail-section message-permission-section"><div className="detail-section-heading"><div><h3>隐私数据查看权限</h3><p>仅授权给确有业务需要的账号，服务端会实时校验当前好友关系。</p></div></div><div className="message-permission-card"><Toggle label="允许查看好友登录 IP" description={currentUser.canViewFriendLoginIp===true?'已授权：可在 PC Web 单聊顶部查看当前好友最近登录 IP 和归属地。':'未授权：PC Web 不展示好友登录 IP 或归属地。'} checked={currentUser.canViewFriendLoginIp===true} disabled={!can('users.write')} onChange={allowed=>{setFriendIPPermission(allowed);setFriendIPPermissionReason('');}}/><div className="message-permission-meta"><span>仅 PC Web</span><span>仅当前好友</span><span>查看行为按小时审计</span></div></div>{!can('users.write')&&<p className="message-permission-readonly">当前管理员没有用户写权限，仅可查看授权状态。</p>}</section>}
      <ConfirmDialog open={deletionPermission!==undefined} title={deletionPermission?'授权全端删除消息':'取消全端删除权限'} detail="权限立即生效，并通知该用户各端刷新。已删除的消息不会恢复。" confirmLabel="确认修改" confirmDisabled={!permissionReason.trim()} onClose={()=>setDeletionPermission(undefined)} onConfirm={async()=>{if(deletionPermission===undefined)return;await api.setUserMessagePermissions(user.id,deletionPermission,permissionReason.trim());setDeletionPermission(undefined);await state.reload();notify('消息删除权限已更新并记录审计');}}><label className="field-label">操作理由<textarea value={permissionReason} maxLength={500} onChange={event=>setPermissionReason(event.target.value)} required/></label></ConfirmDialog>
      <ConfirmDialog open={friendIPPermission!==undefined} title={friendIPPermission?'授权查看好友登录 IP':'取消好友登录 IP 查看权限'} detail="权限变更立即生效并通知该用户刷新；删除好友后也会自动失去查看资格。" confirmLabel="确认修改" confirmDisabled={!friendIPPermissionReason.trim()} onClose={()=>setFriendIPPermission(undefined)} onConfirm={async()=>{if(friendIPPermission===undefined)return;const result=await api.setUserFriendLoginIPPermission(user.id,friendIPPermission,friendIPPermissionReason.trim());setFriendIPPermission(undefined);setFriendIPPermissionReason('');await state.reload();notify(result.changed?'好友登录 IP 查看权限已更新并记录审计':'权限状态未变化，操作已记录');}}><label className="field-label">操作理由<textarea value={friendIPPermissionReason} maxLength={500} onChange={event=>setFriendIPPermissionReason(event.target.value)} required/></label></ConfirmDialog>
...
function GroupDetailDialog({ group, onClose }: { group: GroupRecord; onClose: () => void }) {
  const [historyTarget, setHistoryTarget] = useState<boolean>();
  const [historyReason, setHistoryReason] = useState('');
  const { api, mode, notify, can } = useApi();
  const [section, setSection] = useState<'profile' | 'members' | 'messages' | 'blacklist'>('profile');
  ...
  const [proxyOpen, setProxyOpen] = useState(false); const [proxySender, setProxySender] = useState<GroupMemberRecord>(); const [proxyContent, setProxyContent] = useState(''); const [proxyReason, setProxyReason] = useState('');
  const [blacklistMember, setBlacklistMember] = useState<GroupMemberRecord>(); const [blacklistRemark, setBlacklistRemark] = useState(''); const [blacklistReason, setBlacklistReason] = useState('');
  const [governance, setGovernance] = useState<'mute' | 'unmute' | 'ban' | 'unban'>(); const [governanceReason, setGovernanceReason] = useState('');
  const [beforeSeq, setBeforeSeq] = useState(0); const [messageCursors, setMessageCursors] = useState<number[]>([]); const [recalling, setRecalling] = useState<AdminGroupMessageRecord>(); const [recallReason, setRecallReason] = useState('');
  ...
  const messages = useResource(() => section === 'messages' ? api.getGroupMessages(group.id, beforeSeq, 50) : Promise.resolve({ items: [] as AdminGroupMessageRecord[], nextBeforeSeq: undefined }), [api, mode, group.id, beforeSeq, section]);
  const blacklist = useResource(() => section === 'blacklist' ? api.getGroupBlacklist(group.id) : Promise.resolve([] as AdminGroupBlacklistRecord[]), [api, mode, group.id, section]);
  ...
  const memberUser = (member: GroupMemberRecord): UserRecord => ({ id: member.userId, nickname: member.groupNickname || member.name, phone: member.phone || '未提供', handle: member.handle, avatar: (member.groupNickname || member.name || '?').slice(0, 1), avatarUrl: member.avatarUrl, status: 'active', online: false, onlineConnections: 0, registeredAt: member.joinedAt, lastSeen: '暂无', deviceCount: 0, messageCount: 0, remark: '', tags: [], gender: 'unspecified', handleChangeCount: 0 });
  const sendProxy = async () => { const sender = proxySender?.userId || data?.ownerId; if (!sender || !proxyContent.trim() || !proxyReason.trim()) throw new Error('请选择发送人并填写消息和理由'); await api.sendGroupMessage(group.id, sender, proxyContent.trim(), proxyReason.trim()); notify('群消息已通过真实成员身份发送'); setProxyOpen(false); setProxySender(undefined); setProxyContent(''); setProxyReason(''); await messages.reload(); };
  const addBlacklist = async () => { if (!blacklistMember || !blacklistReason.trim()) throw new Error('请输入拉黑理由'); await api.addGroupBlacklist(group.id, blacklistMember.userId, blacklistRemark.trim(), blacklistReason.trim()); notify('成员已移出并加入群黑名单'); setBlacklistMember(undefined); setBlacklistRemark(''); setBlacklistReason(''); await Promise.all([members.reload(), blacklist.reload(), overview.reload()]); };
  const removeBlacklist = async (item: AdminGroupBlacklistRecord) => { const why = window.prompt(`解除 ${item.user.nickname} 的群黑名单，请填写理由`); if (!why?.trim()) return; await api.removeGroupBlacklist(group.id, item.user.id, why.trim()); notify('群黑名单已解除，不会自动恢复成员身份'); await blacklist.reload(); };
  const executeGovernance = async () => { if (!governance || !governanceReason.trim()) throw new Error('请输入处置理由'); if (governance === 'mute' || governance === 'unmute') await api.setGroupMuteAll(group.id, governance === 'mute', governanceReason.trim()); else await api.setGroupBan(group.id, governance === 'ban', governanceReason.trim()); notify(governance === 'mute' ? '已开启永久全员禁言' : governance === 'unmute' ? '已解除全员禁言' : governance === 'ban' ? '群组已封禁' : '群组已解封'); setGovernance(undefined); setGovernanceReason(''); await overview.reload(); };
  const recallMessage = async () => { if (!recalling || !recallReason.trim()) throw new Error('请输入删除理由'); await api.recallGroupMessage(group.id, recalling.id, recallReason.trim()); notify('消息已全端撤回并写入审计'); setRecalling(undefined); setRecallReason(''); await messages.reload(); };
  const sortedMessages = [...(messages.data?.items ?? [])].sort((left, right) => left.conversationSeq - right.conversationSeq);
  return <><DetailDialog title="群组详情" detail="群资料、成员、WuKongIM 历史消息与黑名单均来自服务端实时数据。" onClose={onClose}>
    {overview.loading && <Skeleton rows={3} />}
    {overview.error && <ErrorState message={overview.error} retry={overview.reload} />}
    {data && <><div className="detail-identity"><span className="group-detail-icon">{data.avatarUrl?<img src={data.avatarUrl} alt=""/>:<Users size={20} />}</span><div><h3>{data.title}</h3><p className="mono">{data.id}</p></div><Badge value={data.dissolvedAt ? 'dissolved' : data.banned ? 'banned' : data.allMutedUntil ? 'muted' : 'active'} /></div>
      ...
      <div className="tabs detail-tabs" role="tablist" onKeyDown={tabListKeyDown}>{([['profile','群资料'],['members',`群成员 ${data.memberCount}`],['messages',`聊天记录 ${data.messageCount}`],['blacklist',`黑名单 ${blacklist.data?.length ?? 0}`]] as const).map(([value,label])=><button type="button" role="tab" aria-selected={section===value} className={section===value?'active':''} key={value} onClick={()=>setSection(value)}>{label}</button>)}</div>
      {section==='profile' && <section className="detail-section user-detail-section"><div className="detail-section-heading"><div><h3>群组资料与治理</h3><p>封禁可恢复；解散仍在列表页的高风险区域执行。</p></div><div className="row-actions wrap"><button className="button secondary compact" disabled={!can('groups.write')||Boolean(data.dissolvedAt)} onClick={()=>{setProxyOpen(true);setProxySender(undefined);setProxyContent('');setProxyReason('');}}>以群主发消息</button><button className="button secondary compact" disabled={!can('groups.write')||Boolean(data.dissolvedAt)} onClick={()=>{setGovernance(data.allMutedUntil?'unmute':'mute');setGovernanceReason('');}}>{data.allMutedUntil?'解除全员禁言':'永久全员禁言'}</button><button className="button secondary compact danger-text" disabled={!can('groups.write')||Boolean(data.dissolvedAt)} onClick={()=>{setGovernance(data.banned?'unban':'ban');setGovernanceReason('');}}>{data.banned?'解封群组':'封禁群组'}</button></div></div><dl className="detail-list"><div className="detail-list-wide"><dt>群主</dt><dd><UserIdentity user={data.owner} fallbackId={data.ownerId}/></dd></div><div><dt>加入方式</dt><dd>{joinPolicyLabels[data.joinPolicy]??data.joinPolicy}</dd></div><div><dt>成员互加好友</dt><dd>{data.allowMemberAddFriend?'允许':'不允许'}</dd></div><div className="detail-list-wide"><dt>新成员可查看入群前历史</dt><dd><button type="button" role="switch" aria-checked={data.historyVisibleToNewMembers} aria-label="新成员可查看入群前历史" className="button secondary compact" disabled={!can('groups.write')||Boolean(data.dissolvedAt)} onClick={()=>{setHistoryTarget(!data.historyVisibleToNewMembers);setHistoryReason('');}}>{data.historyVisibleToNewMembers?'已开启':'已关闭'}</button><small>{data.historyVisibleToNewMembers?'当前成员可查看全部群历史':'仅可查看本人本次入群后的消息，群主和管理员同样受限'}</small></dd></div><div className="detail-list-wide"><dt>群公告</dt><dd>{data.announcement||'尚未设置群公告'}</dd></div>{data.banned&&<div className="detail-list-wide"><dt>封禁依据</dt><dd>{data.banReason||'未记录'}{data.bannedAt&&<small>{dateTimeLabel(data.bannedAt)}</small>}</dd></div>}</dl></section>}
    </>}
    {section==='members' && <section className="detail-section user-detail-section"><div className="detail-section-heading"><div><h3>群成员</h3><p>可代发文本、调整角色、单独禁言、移出或加入黑名单。</p></div><label className="search-field detail-search"><span className="sr-only">搜索群成员</span><Search size={16} /><input aria-label="搜索群成员" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索群成员" /></label></div>
      ...
      {!!members.data?.items.length && <><div className="table-wrap detail-table"><table><thead><tr><th>成员</th><th>角色</th><th>加入时间</th><th>读取进度</th><th><span className="sr-only">成员治理操作</span></th></tr></thead><tbody>{members.data.items.map((member: GroupMemberRecord) => <tr key={member.userId}><td><UserIdentity user={memberUser(member)} fallbackId={member.userId}/></td><td><Badge value="active" label={groupRoleLabels[member.role] ?? member.role} />{member.mutedUntil && <small>禁言至 {dateTimeLabel(member.mutedUntil)}</small>}</td><td>{member.joinedAt}</td><td>{member.lastReadSeq.toLocaleString()} / {member.lastDeliveredSeq.toLocaleString()}</td><td><div className="row-actions wrap"><button className="button secondary compact" disabled={!can('groups.write')||group.status==='dissolved'} onClick={()=>{setProxyOpen(true);setProxySender(member);setProxyContent('');setProxyReason('');}}>代发消息</button>{member.role !== 'owner' && <><button className="button secondary compact" disabled={!can('groups.write')} onClick={() => roleAction(member)}>{member.role === 'admin' ? '取消管理员' : '设为管理员'}</button><button className="button secondary compact" disabled={!can('groups.write')} onClick={() => muteAction(member)}>{member.mutedUntil ? '解除禁言' : '禁言 1 小时'}</button><button className="button secondary compact danger-text" disabled={!can('groups.write')} onClick={()=>{setBlacklistMember(member);setBlacklistRemark('');setBlacklistReason('');}}>加入黑名单</button><button className="button secondary compact danger-text" disabled={!can('groups.write')} onClick={() => removeAction(member)}>移出群聊</button></>}</div></td></tr>)}</tbody></table></div><Pagination data={members.data} onPage={paginate} /></>}
    </section>
    }
    {section==='messages'&&<section className="detail-section user-detail-section"><div className="detail-section-heading"><div><h3>群聊记录</h3><p>正文实时从 WuKongIM 拉取，每次查看都会写入审计。</p></div></div>{messages.loading&&<Skeleton rows={5}/>} {messages.error&&<ErrorState message={messages.error} retry={messages.reload}/>} {messages.data&&!sortedMessages.length&&<EmptyState icon={<MessageSquareText size={24}/>} title="群内暂无消息" detail="当前历史页没有可显示的消息。"/>}{!!sortedMessages.length&&<div className="conversation-history">{sortedMessages.map(message=>{const unavailable=Boolean(message.deletedForEveryoneAt||message.recalledAt||message.expiredAt||message.adminRecall);const mediaURL=messageMediaURL(message);return <article className="conversation-message" key={message.id}><UserIdentity user={message.sender} fallbackId={message.senderId}/><div className="conversation-message-main"><header><span>{message.createdAt}</span></header><div className={`conversation-message-body ${unavailable?'muted':''}`}>{message.deletedForEveryoneAt?messageBodySummary(message):message.adminRecall?'该消息已被管理员删除（全端撤回）':message.recalledAt?'该消息已撤回':message.expiredAt?'该消息已过期':messageBodySummary(message)}</div>{mediaURL&&(!unavailable||message.deletedForEveryoneAt)&&<a className="conversation-attachment" href={mediaURL} target="_blank" rel="noreferrer">查看附件</a>}{message.adminRecall&&<small>处置人：{message.moderatedBy||'管理员'} · {message.moderationReason||'未提供理由'}</small>}</div><button className="button secondary compact danger-text" disabled={unavailable||!can('groups.write')} onClick={()=>{setRecalling(message);setRecallReason('');}}>删除（全端撤回）</button></article>})}</div>}<div className="conversation-pagination"><button className="button secondary compact" disabled={!messageCursors.length} onClick={()=>setMessageCursors(current=>{const next=[...current];setBeforeSeq(next.pop()??0);return next;})}>返回较新消息</button><span>当前显示 {sortedMessages.length} 条</span><button className="button secondary compact" disabled={!messages.data?.nextBeforeSeq} onClick={()=>{setMessageCursors(current=>[...current,beforeSeq]);setBeforeSeq(messages.data?.nextBeforeSeq??0);}}>加载更早消息</button></div></section>}
    {section==='blacklist'&&<section className="detail-section user-detail-section"><div className="detail-section-heading"><div><h3>群黑名单</h3><p>解除后只恢复加入资格，不会自动重新入群。</p></div></div>{blacklist.loading&&<Skeleton rows={4}/>} {blacklist.error&&<ErrorState message={blacklist.error} retry={blacklist.reload}/>} {blacklist.data&&!blacklist.data.length&&<EmptyState icon={<ShieldAlert size={24}/>} title="群黑名单为空" detail="可从群成员列表将违规成员移出并加入黑名单。"/>}{!!blacklist.data?.length&&<div className="table-wrap detail-table"><table><thead><tr><th>用户</th><th>操作者</th><th>备注</th><th>拉黑时间</th><th></th></tr></thead><tbody>{blacklist.data.map(item=><tr key={item.user.id}><td><UserIdentity user={item.user} fallbackId={item.user.id}/></td><td><strong>{item.operatorName}</strong><small className="mono">{item.operatorId}</small></td><td>{item.remark||'未记录'}</td><td>{item.createdAt}</td><td><button className="button secondary compact" disabled={!can('groups.write')} onClick={()=>void removeBlacklist(item)}>解除黑名单</button></td></tr>)}</tbody></table></div>}</section>}
  </DetailDialog>
  <ConfirmDialog open={historyTarget !== undefined} title={historyTarget ? "开放入群前历史" : "隐藏入群前历史"} detail="对所有当前成员生效，不影响后台审核。已导出文件、截图和旧版离线副本无法收回。" confirmLabel="确认修改" confirmDisabled={!historyReason.trim()} onClose={()=>setHistoryTarget(undefined)} onConfirm={async()=>{if(historyTarget===undefined||!historyReason.trim())return;await api.setGroupHistoryVisibility(group.id,historyTarget,historyReason.trim());setHistoryTarget(undefined);await overview.reload();notify("群历史可见性已更新并写入审计");}}><label className="field-label">操作理由<textarea value={historyReason} maxLength={500} onChange={event=>setHistoryReason(event.target.value)} required/></label></ConfirmDialog>
  <ConfirmDialog open={Boolean(moderation)} title={moderation?.title ?? ''} detail={moderation?.detail ?? ''} confirmLabel={moderation?.confirmLabel ?? '确认处置'} danger={moderation?.danger} confirmDisabled={!reason.trim()} onClose={() => { setModeration(undefined); setReason(''); }} onConfirm={executeModeration}><label className="field-label">处置理由<textarea value={reason} maxLength={500} onChange={(event) => setReason(event.target.value)} placeholder="填写违规证据、运营工单或处置依据" required /></label></ConfirmDialog>
  <ConfirmDialog open={proxyOpen} title="使用真实群成员身份发送消息" detail={`发送人：${proxySender?.name||data?.owner.nickname||'群主'}。消息仅携带 adminProxy 标记，管理员身份和理由只进入审计。`} confirmLabel="确认发送" confirmDisabled={!proxyContent.trim()||!proxyReason.trim()} onClose={()=>{setProxyOpen(false);setProxySender(undefined);setProxyContent('');setProxyReason('');}} onConfirm={sendProxy}><label className="field-label">消息内容<textarea value={proxyContent} maxLength={5000} onChange={event=>setProxyContent(event.target.value)} required/></label><label className="field-label">操作理由<textarea value={proxyReason} maxLength={500} onChange={event=>setProxyReason(event.target.value)} required/></label></ConfirmDialog>
  <ConfirmDialog open={Boolean(blacklistMember)} title="移出并加入群黑名单" detail={blacklistMember?`${blacklistMember.name} 将立即移出群聊，待处理邀请会取消，并禁止再次邀请、扫码和接受旧邀请。`:''} confirmLabel="确认加入黑名单" danger confirmDisabled={!blacklistReason.trim()} onClose={()=>setBlacklistMember(undefined)} onConfirm={addBlacklist}><label className="field-label">黑名单备注<textarea value={blacklistRemark} maxLength={500} onChange={event=>setBlacklistRemark(event.target.value)} placeholder="可记录违规类型或关系备注"/></label><label className="field-label">处置理由<textarea value={blacklistReason} maxLength={500} onChange={event=>setBlacklistReason(event.target.value)} required/></label></ConfirmDialog>
  <ConfirmDialog open={Boolean(governance)} title={governance==='mute'?'永久全员禁言':governance==='unmute'?'解除全员禁言':governance==='ban'?'封禁群组':'解封群组'} detail={governance==='mute'?'群主和管理员仍可发言。':governance==='ban'?'保留成员和历史消息，但频道将停止通信。':'操作将立即同步到 WuKongIM。'} confirmLabel="确认操作" danger={governance==='mute'||governance==='ban'} confirmDisabled={!governanceReason.trim()} onClose={()=>setGovernance(undefined)} onConfirm={executeGovernance}><label className="field-label">操作理由<textarea value={governanceReason} maxLength={500} onChange={event=>setGovernanceReason(event.target.value)} required/></label></ConfirmDialog>
  <ConfirmDialog open={Boolean(recalling)} title="删除群消息（全端撤回）" detail="WuKongIM 原始消息不会物理删除，客户端会同步显示为已撤回。" confirmLabel="确认全端撤回" danger confirmDisabled={!recallReason.trim()} onClose={()=>setRecalling(undefined)} onConfirm={recallMessage}><label className="field-label">删除理由<textarea value={recallReason} maxLength={500} onChange={event=>setRecallReason(event.target.value)} required/></label></ConfirmDialog></>;
}
...
type NavigationGroup = { id: string; label: string; icon: typeof Users; items: NavigationItem[] };
const navGroups: NavigationGroup[] = [
  {
    id: 'users', label: '用户与关系', icon: Users, items: [
      { to: '/users/new', label: '新增用户', icon: Plus },
      { to: '/users', label: '用户管理', icon: Users },
      { to: '/users/banned', label: '封禁用户', icon: ShieldAlert },
      { to: '/invitations', label: '邀请管理', icon: LockKeyhole },
  ...
  {
    id: 'messages', label: '群聊与消息', icon: MessageSquareText, items: [
      { to: '/groups', label: '群组列表', icon: Group },
      { to: '/groups/banned', label: '封禁群列表', icon: ShieldAlert },
      { to: '/messages', label: '消息检索', icon: MessageSquareText },
      { to: '/calls', label: '通话记录', icon: PhoneCall },
    ],
  },
  {
    id: 'reports', label: '内容安全', icon: ShieldCheck, items: [
  ...
  {
    id: 'operations', label: '运营服务', icon: Activity, items: [
  ...
  {
    id: 'infrastructure', label: '基础设施', icon: Server, items: [
  ...
  {
    id: 'settings', label: '系统管理', icon: Settings, items: [
...
function Shell() {
  ...
  const searchableNavigation = useMemo(() => [
    { ...overviewNavItem, group: '概览' },
    ...visibleNavGroups.flatMap((group) => group.items.map((item) => ({ ...item, group: group.label }))),
  ], [visibleNavGroups]);
  const normalizedGlobalSearch = globalSearch.trim().toLocaleLowerCase();
  const globalSearchResults = normalizedGlobalSearch
    ? searchableNavigation.filter((item) => `${item.label} ${item.group} ${item.to}`.toLocaleLowerCase().includes(normalizedGlobalSearch)).slice(0, 8)
    : [];
  const activeGroupId = visibleNavGroups.find((group) => group.items.some((item) => item.to === path))?.id;
  const activeNavigationLabel = path === overviewNavItem.to
    ? overviewNavItem.label
    : visibleNavGroups.flatMap((group) => group.items).find((item) => item.to === path)?.label ?? '运营控制台';
  useEffect(() => {
      ...
      <nav aria-label="主导航">
        ...
        {visibleNavGroups.map((group) => {
          ...
          return <section className="nav-section" key={group.id}>
            <button type="button" className={`nav-section-toggle ${hasActive ? 'has-active' : ''}`} aria-expanded={expanded} aria-controls={`nav-group-${group.id}`} onClick={() => { if (sidebarCollapsed) setSidebarCollapsed(false); setExpandedNavGroups((current) => ({ ...current, [group.id]: !expanded })); }}><GroupIcon className="nav-section-icon" size={18} aria-hidden="true" /><span>{group.label}</span><ChevronDown className="nav-section-chevron" size={15} /></button>
            <div id={`nav-group-${group.id}`} className="nav-section-items" hidden={!expanded}>{group.items.map((item) => <AppLink key={item.to} to={item.to} currentPath={path} navigate={requestNavigate} className="nav-item nav-leaf-item"><item.icon size={15} aria-hidden="true" /><span>{item.label}</span></AppLink>)}</div>
          </section>;
        })}
      </nav>
      <div className="sidebar-foot"><div className="admin-avatar" aria-hidden="true">{[...session.displayName][0]}</div><div className="sidebar-account-copy"><strong>@{session.username}</strong><span>{session.email || session.displayName}</span></div><div className="sidebar-account-actions"><button className="icon-button sidebar-logout" aria-label="修改密码" title="修改密码" onClick={() => requestNavigate('/change-password')}><LockKeyhole size={17} /></button><button className="icon-button sidebar-logout" aria-label="退出管理后台" title="退出登录" onClick={requestLogout}><LogOut size={17} /></button></div></div>
    </aside>
    <div className="workspace">
      <div className="topbar">
        <div className="topbar-primary">
          <button ref={menuButtonRef} className="icon-button menu-button" aria-label={compactNavigation ? '打开导航' : sidebarCollapsed ? '展开侧栏' : '收起侧栏'} aria-controls="admin-navigation" aria-expanded={compactNavigation ? navOpen : !sidebarCollapsed} onClick={() => compactNavigation ? setNavOpen(true) : setSidebarCollapsed((value) => !value)}>{compactNavigation ? <Menu size={20} /> : sidebarCollapsed ? <ChevronRight size={19} /> : <ChevronLeft size={19} />}</button>
          <div className="global-search-shell">
            <Search size={18} aria-hidden="true" />
            <input ref={globalSearchRef} type="search" value={globalSearch} onChange={(event) => setGlobalSearch(event.target.value)} aria-label="搜索后台功能" placeholder="搜索功能或输入命令…" autoComplete="off" />
            <kbd>⌘ K</kbd>
        ...
        <div className="topbar-actions">
          <div className="topbar-status"><span className="pulse-dot" /><span>实时数据</span></div>
          <div className="topbar-user"><span className="admin-avatar">{[...session.displayName][0]}</span><span><strong>{session.displayName}</strong><small>@{session.username} · {session.roleName || roleLabels[session.roleId] || session.roleId}</small></span></div>
        </div>
      </div>
...
const dashboardMetricIcons = [Users, MessageSquareText, ShieldAlert, Activity];

function OverviewPage() {
  const { api, mode } = useApi();
  const state = useResource(() => api.getDashboard(), [api, mode]);
  return <><PageHeader title="运行概览" description="所有指标均由服务端按当前业务数据实时聚合，不使用演示数据。" actions={<button className="button secondary" onClick={() => void state.reload()}><RefreshCcw size={15} />刷新数据</button>} />
    {state.loading ? <Skeleton rows={6} /> : state.error || !state.data ? <ErrorState message={state.error} retry={state.reload} /> : <div className="dashboard">
      <div className="dashboard-source" role="status"><span className="pulse-dot" />数据来源：生产业务数据库{state.data.generatedAt ? ` · 更新于 ${state.data.generatedAt}` : ' · 当前接口响应'}</div>
      <section className="metric-strip" aria-label="核心指标">{state.data.metrics.map((item, index) => { const MetricIcon = dashboardMetricIcons[index % dashboardMetricIcons.length]; return <article className="metric" key={item.label}><div className="metric-heading"><span>{item.label}</span><span className="metric-icon" aria-hidden="true"><MetricIcon size={16} strokeWidth={1.8} /></span></div><strong>{item.value}</strong><div className="metric-foot"><p>{item.delta}</p><Badge value={item.tone} label={metricToneLabels[item.tone]} /></div></article>; })}</section>
      <section className="dashboard-grid">
        <div className="panel trend-panel"><div className="panel-heading"><div><h2>消息流量</h2><p>最近 {state.data.trendWindowHours ?? 24} 小时服务端逐小时统计</p></div></div><TrendChart data={state.data.messageTrend} /></div>
        <div className="panel mix-panel"><div className="panel-heading"><div><h2>消息构成</h2><p>最近 {state.data.mixWindowHours ?? 24} 小时按会话类型统计</p></div></div>{state.data.channelMix.length ? <><div className="donut" style={{ '--segments': channelMixGradient(state.data.channelMix) } as React.CSSProperties}><div><strong>{Math.round(state.data.channelMix.reduce((sum, item) => sum + item.value, 0))}%</strong><span>已分类</span></div></div><div className="mix-legend">{state.data.channelMix.map((item) => <div key={item.label}><span style={{ background: item.color }} /><b>{item.label}</b><strong>{item.value}%</strong></div>)}</div></> : <div className="panel-empty">最近 24 小时暂无消息构成数据</div>}</div>
...
function Toolbar({ query, setQuery, placeholder, children }: { query: string; setQuery: (value: string) => void; placeholder: string; children?: ReactNode }) {
  return <div className="toolbar data-toolbar" role="search"><label className="search-field"><span className="sr-only">{placeholder}</span><Search size={17} /><input aria-label={placeholder} value={query} onChange={(event) => setQuery(event.target.value)} placeholder={placeholder} /></label>{children}</div>;
}
...
function UserDeviceSummary({ user }: { user: UserRecord }) {
  const deviceTitle = user.latestDevice?.deviceName || user.latestDevice?.deviceModel || user.latestDevice?.platform || '暂无设备上报';
  const deviceMeta = user.latestDevice
    ? `${user.latestDevice.platform} · App ${user.latestDevice.appVersion || '未知'}${user.latestDevice.lastSeenAt ? ` · 最后活跃 ${user.latestDevice.lastSeenAt}` : ''}`
    : '等待新版客户端上报';
  return <div className="device-summary"><strong title={deviceTitle}>{deviceTitle}</strong><small title={deviceMeta}>{deviceMeta}</small></div>;
}

function CompactPermissionSwitch({ checked, disabled, onChange }: { checked: boolean; disabled: boolean; onChange: (checked: boolean) => void }) {
  const label = checked ? '已授权' : '未授权';
  return <label className={`compact-permission-switch${disabled ? ' disabled' : ''}`} title={disabled ? `好友登录 IP 查看权限：${label}（当前管理员无修改权限）` : `好友登录 IP 查看权限：${label}`}>
    <input type="checkbox" role="switch" aria-label="允许查看好友登录 IP" checked={checked} disabled={disabled} onChange={event => onChange(event.target.checked)} />
    <span className="compact-permission-switch-control" aria-hidden="true" />
    <span className="compact-permission-switch-label">{label}</span>
  </label>;
}

function UserManagementCard({ user, canWrite, selectedForIP, onSelectedForIP, onFriendIPPermissionChange, onDetail, onRecords, onMessage, onDisposition,onIP }: { user: UserRecord; canWrite: boolean; selectedForIP: boolean; onSelectedForIP: (selected:boolean) => void; onFriendIPPermissionChange: (allowed:boolean) => void; onDetail: () => void; onRecords: () => void; onMessage: () => void; onDisposition: () => void;onIP:(ip:string)=>void }) {
  const {notify}=useApi();
  return <article className="user-management-card">
    <label className="user-card-selection"><input type="checkbox" checked={selectedForIP} disabled={!canWrite} onChange={event=>onSelectedForIP(event.target.checked)}/>选择此账号进行批量权限设置</label>
    <button type="button" className="user-card-identity" onClick={onDetail} aria-label={`查看 ${user.nickname} 的完整资料`}>
      <UserAvatar user={user} />
      <span><strong>{user.nickname}</strong><small>{user.phone}</small></span>
      <Badge value={user.status} />
    </button>
    <dl className="user-card-facts">
      <div><dt>在线状态</dt><dd><Badge value={user.online ? 'active' : 'neutral'} label={user.online ? `在线 · ${user.onlineConnections || 1}` : '离线'} /></dd><small>{user.online ? '当前连接正常' : user.lastOfflineAt ? `最后离线 ${dateTimeLabel(user.lastOfflineAt)}` : '暂无离线记录'}</small></div>
      <div><dt>好友 IP 查看授权</dt><dd><CompactPermissionSwitch checked={user.canViewFriendLoginIp === true} disabled={!canWrite} onChange={onFriendIPPermissionChange} /></dd></div>
      <div><dt>注册时间</dt><dd>{user.registeredAt}</dd>{user.bannedUntil && <small>封禁至 {dateTimeLabel(user.bannedUntil)}</small>}</div>
      <div><dt>注册 IP / 地址</dt><dd><IPValue ip={user.access?.registrationIp} region={user.access?.registrationRegion} onIP={onIP} notify={notify}/></dd></div>
      <div><dt>登录 IP / 地址</dt><dd><IPValue ip={user.access?.lastLoginIp} region={user.access?.lastLoginRegion} onIP={onIP} notify={notify}/></dd><small>{user.access?.lastLoginAt ? `最近登录 ${dateTimeLabel(user.access.lastLoginAt)}` : '登录时间未记录'}</small></div>
      <div className="user-card-device-fact"><dt>最近设备</dt><dd><UserDeviceSummary user={user} /></dd></div>
    </dl>
    <div className="user-card-actions"><button type="button" className="button primary compact" onClick={onDetail}>查看完整资料</button><button type="button" className="button secondary compact" onClick={onRecords}>消息记录</button><button type="button" className="button secondary compact" disabled={!canWrite} onClick={onMessage}>发系统消息</button><button type="button" className="button secondary compact danger-text" disabled={!canWrite} onClick={onDisposition}>{user.status === 'banned' ? '解除封禁' : '封禁账号'}</button></div>
  </article>;
}

function UsersPage({ scope = 'all' }: { scope?: 'all' | 'banned' }) {
  const { api, mode, notify, can } = useApi();
  const [logsOpen,setLogsOpen]=useState(false);const [selectedIP,setSelectedIP]=useState<string>();
  const [ipDraft,setIPDraft]=useState('');const [ip,setIP]=useState('');const [ipSource,setIPSource]=useState('any');
  const [friendIPPermissionFilter,setFriendIPPermissionFilter]=useState('');
  const [selectedUserIds,setSelectedUserIds]=useState<string[]>([]);const [batchFriendIPPermission,setBatchFriendIPPermission]=useState<boolean>();const [batchFriendIPReason,setBatchFriendIPReason]=useState('');
  const [friendIPPermissionChange,setFriendIPPermissionChange]=useState<{user:UserRecord;allowed:boolean}>();const [friendIPPermissionReason,setFriendIPPermissionReason]=useState('');
  const narrowList = useMediaQuery('(max-width: 1023px)');
  const [query, setQuery] = useState(() => new URLSearchParams(window.location.search).get('q') ?? ''), deferredQuery = useDebouncedValue(query); const [status, setStatus] = useState(scope === 'banned' ? 'banned' : ''); const [page, setPage] = useState(1); const [pageSize, setPageSize] = useState(20);
  ...
  useEffect(() => { setPage(1); setCursors({ 1: '' }); }, [deferredQuery, status, pageSize,ip,ipSource,friendIPPermissionFilter]);
  useEffect(()=>{setSelectedUserIds([]);setBatchFriendIPPermission(undefined);setBatchFriendIPReason('');},[page,deferredQuery,status,pageSize,ip,ipSource,friendIPPermissionFilter]);
  const state = useResource(() => api.getUsers(deferredQuery, status, page, pageSize, cursors[page] ?? '',ip,ipSource,friendIPPermissionFilter), [api, mode, deferredQuery, status, page, pageSize, cursors,ip,ipSource,friendIPPermissionFilter]);
  ...
  const pageUserIds=state.data?.items.map(user=>user.id)??[];const selectedPageUserIds=pageUserIds.filter(id=>selectedUserIds.includes(id));const allPageUsersSelected=pageUserIds.length>0&&selectedPageUserIds.length===pageUserIds.length;
  const setUserSelected=(id:string,selected:boolean)=>setSelectedUserIds(current=>selected?[...new Set([...current,id])]:current.filter(candidate=>candidate!==id));
  const applyBatchFriendIPPermission=async()=>{if(batchFriendIPPermission===undefined||!selectedPageUserIds.length||!batchFriendIPReason.trim())throw new Error('请选择当前页用户并填写操作理由');const result=await api.setUsersFriendLoginIPPermission(selectedPageUserIds,batchFriendIPPermission,batchFriendIPReason.trim());notify(`权限处理完成：更新 ${result.changed} 人，未变化 ${result.unchanged} 人`);setBatchFriendIPPermission(undefined);setBatchFriendIPReason('');setSelectedUserIds([]);await state.reload();};
  const openFriendIPPermissionChange=(user:UserRecord,allowed:boolean)=>{setFriendIPPermissionChange({user,allowed});setFriendIPPermissionReason('');};
  const applyFriendIPPermissionChange=async()=>{if(!friendIPPermissionChange||!friendIPPermissionReason.trim())throw new Error('请填写操作理由');const result=await api.setUserFriendLoginIPPermission(friendIPPermissionChange.user.id,friendIPPermissionChange.allowed,friendIPPermissionReason.trim());notify(result.changed?'好友登录 IP 查看权限已更新':'权限状态未变化，操作已记录');setFriendIPPermissionChange(undefined);setFriendIPPermissionReason('');await state.reload();};
  if(selectedIP)return <IPAccountsDialog ip={selectedIP} onClose={()=>setSelectedIP(undefined)}/>;
  ...
  return <><PageHeader title={scope === 'banned' ? '封禁用户' : '用户管理'} description={scope === 'banned' ? '集中查看当前仍处于封禁状态的账号，可进入完整资料核对后解除封禁。' : '查询真实账号资料，查看好友、黑名单和设备，并执行消息通知与账号治理。'} actions={<><button className="button secondary" onClick={()=>setLogsOpen(true)}>登录日志</button><button type="button" className="button primary" disabled={!can('users.write')} onClick={() => { window.history.pushState({}, '', '/users/new'); window.dispatchEvent(new PopStateEvent('popstate')); }}><Plus size={16} />新增用户</button></>} />
    <Toolbar query={query} setQuery={setQuery} placeholder="搜索昵称、用户 ID、手机号或呱呱号">{scope === 'all' && <select aria-label="用户状态" className="select-control" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">全部状态</option><option value="active">账号正常</option><option value="banned">已封禁</option></select>}<select aria-label="好友 IP 查看权限" className="select-control" value={friendIPPermissionFilter} onChange={event=>setFriendIPPermissionFilter(event.target.value)}><option value="">全部 IP 查看权限</option><option value="allowed">已授权查看</option><option value="denied">未授权查看</option></select><select aria-label="每页数量" className="select-control" value={pageSize} onChange={(event) => setPageSize(Number(event.target.value))}><option value={20}>每页 20 条</option><option value={50}>每页 50 条</option><option value={100}>每页 100 条</option></select></Toolbar>
    ...
    {!!selectedPageUserIds.length&&<div className="user-permission-batch-bar" role="region" aria-label="批量设置好友 IP 查看权限"><strong>已选择 {selectedPageUserIds.length} 个当前页账号</strong><span>权限修改将立即生效并记录审计。</span><div className="row-actions"><button type="button" className="button primary compact" disabled={!can('users.write')} onClick={()=>{setBatchFriendIPPermission(true);setBatchFriendIPReason('');}}>批量授权</button><button type="button" className="button secondary compact" disabled={!can('users.write')} onClick={()=>{setBatchFriendIPPermission(false);setBatchFriendIPReason('');}}>批量取消</button><button type="button" className="button secondary compact" onClick={()=>setSelectedUserIds([])}>清除选择</button></div></div>}
    <DataPanel loading={state.loading} error={state.error} retry={state.reload} empty={!state.data?.items.length} emptyTitle="没有匹配的用户" emptyDetail="调整搜索词或状态筛选后重试。"><>
      {narrowList ? <div className="user-card-list">{state.data?.items.map((user) => <UserManagementCard key={user.id} user={user} canWrite={can('users.write')} selectedForIP={selectedUserIds.includes(user.id)} onSelectedForIP={selected=>setUserSelected(user.id,selected)} onFriendIPPermissionChange={allowed=>openFriendIPPermissionChange(user,allowed)} onIP={setSelectedIP} onDetail={() => setDetailUser(user)} onRecords={() => viewMessageRecords(user)} onMessage={() => openMessage(user)} onDisposition={() => openDisposition(user)} />)}</div> : <div className="table-wrap user-table-desktop"><table className="users-table"><thead><tr><th className="table-select-cell"><input type="checkbox" aria-label="选择当前页全部用户" checked={allPageUsersSelected} disabled={!can('users.write')||!pageUserIds.length} onChange={event=>setSelectedUserIds(event.target.checked?pageUserIds:[])}/></th><th>用户</th><th>账号状态</th><th>好友 IP 授权</th><th>在线状态</th><th>最近设备</th><th>注册 IP / 地址</th><th>登录 IP / 地址</th><th className="table-actions-heading">操作</th></tr></thead><tbody>{state.data?.items.map((user) => <tr key={user.id}><td className="table-select-cell"><input type="checkbox" aria-label={`选择 ${user.nickname}`} checked={selectedUserIds.includes(user.id)} disabled={!can('users.write')} onChange={event=>setUserSelected(user.id,event.target.checked)}/></td><td><button type="button" className="identity identity-button" onClick={() => setDetailUser(user)} aria-label={`查看 ${user.nickname} 的完整资料`}><UserAvatar user={user} /><span><strong title={user.nickname}>{user.nickname}</strong><small>{user.phone}</small></span></button></td><td><Badge value={user.status} />{user.bannedUntil && <small>至 {dateTimeLabel(user.bannedUntil)}</small>}</td><td className="friend-ip-permission-cell"><CompactPermissionSwitch checked={user.canViewFriendLoginIp === true} disabled={!can('users.write')} onChange={allowed=>openFriendIPPermissionChange(user,allowed)} /></td><td><Badge value={user.online ? 'active' : 'neutral'} label={user.online ? `在线 · ${user.onlineConnections || 1}` : '离线'} />{!user.online && <small>{user.lastOfflineAt ? `最后离线 ${dateTimeLabel(user.lastOfflineAt)}` : '暂无离线记录'}</small>}</td><td><UserDeviceSummary user={user} /></td><td className="user-ip-table-cell"><IPValue ip={user.access?.registrationIp} region={user.access?.registrationRegion} onIP={setSelectedIP} notify={notify}/><small>注册时间 {user.registeredAt}</small></td><td className="user-ip-table-cell"><IPValue ip={user.access?.lastLoginIp} region={user.access?.lastLoginRegion} onIP={setSelectedIP} notify={notify}/><small>{user.access?.lastLoginAt ? `最近登录 ${dateTimeLabel(user.access.lastLoginAt)}` : '登录时间未记录'}</small></td><td className="table-actions-cell"><div className="row-actions user-actions"><button className="row-action" aria-label="查看详情" title="查看详情" onClick={() => setDetailUser(user)}>详情</button><button className="row-action" aria-label="查看发送记录" title="查看该用户发送记录" onClick={() => viewMessageRecords(user)}>记录</button><button className="row-action" aria-label="发系统消息" disabled={!can('users.write')} title={!can('users.write') ? '当前角色没有系统消息权限' : '发送系统消息'} onClick={() => openMessage(user)}>消息</button><button className="row-action" aria-label={user.status === 'banned' ? '解除封禁' : '封禁账号'} disabled={!can('users.write')} title={!can('users.write') ? '当前角色没有账号处置权限' : user.status === 'banned' ? '解除封禁' : '封禁账号'} onClick={() => openDisposition(user)}>{user.status === 'banned' ? '解封' : '封禁'}</button></div></td></tr>)}</tbody></table></div>}
      <Pagination data={state.data} onPage={paginate} />
    </></DataPanel>
    {detailUser && <UserDetailDialog user={detailUser} onClose={() => setDetailUser(undefined)} />}
    <ConfirmDialog open={Boolean(friendIPPermissionChange)} title={friendIPPermissionChange?.allowed?'授权查看好友登录 IP':'取消好友登录 IP 查看权限'} detail={friendIPPermissionChange?`目标账号：${friendIPPermissionChange.user.nickname}（${friendIPPermissionChange.user.id}）。变更立即生效并记录审计。`:''} confirmLabel="确认修改" confirmDisabled={!friendIPPermissionReason.trim()} onClose={()=>{setFriendIPPermissionChange(undefined);setFriendIPPermissionReason('');}} onConfirm={applyFriendIPPermissionChange}><label className="field-label">操作理由<textarea value={friendIPPermissionReason} maxLength={500} onChange={event=>setFriendIPPermissionReason(event.target.value)} required/></label></ConfirmDialog>
    <ConfirmDialog open={batchFriendIPPermission!==undefined&&selectedPageUserIds.length>0} title={batchFriendIPPermission?'批量授权查看好友登录 IP':'批量取消好友登录 IP 查看权限'} detail={`当前页已选择 ${selectedPageUserIds.length} 个账号；整批校验，任一账号无效时不会修改任何账号。`} confirmLabel="确认批量修改" confirmDisabled={!batchFriendIPReason.trim()||!selectedPageUserIds.length} onClose={()=>setBatchFriendIPPermission(undefined)} onConfirm={applyBatchFriendIPPermission}><label className="field-label">操作理由<textarea value={batchFriendIPReason} maxLength={500} onChange={event=>setBatchFriendIPReason(event.target.value)} required/></label></ConfirmDialog>
...
function InvitationsPage() {
  const { api, mode, notify, can } = useApi();
  const [tab, setTab] = useState<'codes' | 'relations'>('codes');
  const [query, setQuery] = useState(''), deferredQuery = useDebouncedValue(query);
  const [status, setStatus] = useState(''), [method, setMethod] = useState('');
  const [from, setFrom] = useState(''), [to, setTo] = useState('');
  const [page, setPage] = useState(1), [cursors, setCursors] = useState<Record<number, string>>({ 1: '' });
  const [operation, setOperation] = useState<{ item: InviteCodeRecord; kind: 'enable' | 'disable' | 'reset' }>();
  const [reason, setReason] = useState('');
  useEffect(() => { setPage(1); setCursors({ 1: '' }); }, [tab, deferredQuery, status, method, from, to]);
  const codes = useResource(
    () => tab === 'codes' ? api.getInviteCodes(deferredQuery, status, page, 20, cursors[page] ?? '') : Promise.resolve({ items: [] as InviteCodeRecord[], page: 1, pageSize: 20, total: 0, hasNext: false, nextCursor: undefined }),
    [api, mode, tab, deferredQuery, status, page, cursors],
  );
  const relations = useResource(
    () => tab === 'relations' ? api.getInviteRelations(deferredQuery, method, from, nextDayIso(to), page, 20, cursors[page] ?? '') : Promise.resolve({ items: [] as InviteRelationRecord[], page: 1, pageSize: 20, total: 0, hasNext: false, nextCursor: undefined }),
    [api, mode, tab, deferredQuery, method, from, to, page, cursors],
  );
  const currentCursor = tab === 'codes' ? codes.data?.nextCursor : relations.data?.nextCursor;
  const paginate = (next: number) => { if (next > page && currentCursor) setCursors((value) => ({ ...value, [next]: currentCursor })); setPage(next); };
  const startOperation = (item: InviteCodeRecord, kind: 'enable' | 'disable' | 'reset') => { setReason(''); setOperation({ item, kind }); };
  const executeOperation = async () => {
    if (!operation || !reason.trim()) throw new Error('请输入操作理由');
    if (operation.kind === 'reset') await api.resetInviteCode(operation.item.id, reason.trim());
    else await api.setInviteCodeStatus(operation.item.id, operation.kind === 'enable' ? 'active' : 'disabled', reason.trim());
    notify(operation.kind === 'reset' ? '邀请码已重置，旧码永久退休' : operation.kind === 'enable' ? '邀请码已启用' : '邀请码已停用');
    setOperation(undefined); setReason(''); await codes.reload();
  };
  const operationTitle = operation?.kind === 'reset' ? '重置邀请码' : operation?.kind === 'enable' ? '启用邀请码' : '停用邀请码';
  return <><PageHeader title="邀请管理" description="管理注册邀请码和邀请来源；补绑或改绑请前往用户详情，所有变更记录审计。" />
    <div className="tabs" role="tablist" onKeyDown={tabListKeyDown}><button type="button" role="tab" aria-selected={tab === 'codes'} className={tab === 'codes' ? 'active' : ''} onClick={() => setTab('codes')}>邀请码</button><button type="button" role="tab" aria-selected={tab === 'relations'} className={tab === 'relations' ? 'active' : ''} onClick={() => setTab('relations')}>邀请关系</button></div>
    <Toolbar query={query} setQuery={setQuery} placeholder="搜索邀请码、昵称、用户 ID、手机号或呱呱号">{tab === 'codes' ? <select className="select-control" aria-label="邀请码状态" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">全部状态</option><option value="active">正常</option><option value="disabled">已停用</option><option value="retired">已退休</option></select> : <><select className="select-control" aria-label="绑定方式" value={method} onChange={(event) => setMethod(event.target.value)}><option value="">全部绑定方式</option><option value="password">密码注册</option><option value="otp">验证码开户</option><option value="admin">后台补绑</option></select><input aria-label="绑定开始日期" type="date" value={from} onChange={(event) => setFrom(event.target.value)} /><input aria-label="绑定结束日期" type="date" value={to} onChange={(event) => setTo(event.target.value)} /></>}</Toolbar>
    {tab === 'codes' ? <DataPanel loading={codes.loading} error={codes.error} retry={codes.reload} empty={!codes.data?.items.length} emptyTitle="没有匹配的邀请码" emptyDetail="调整邀请码、用户或状态筛选后重试。"><><div className="table-wrap"><table><thead><tr><th>用户</th><th>邀请码</th><th>状态</th><th>自主修改</th><th>生成来源</th><th>创建时间</th><th></th></tr></thead><tbody>{codes.data?.items.map((item) => <tr key={item.id}><td><UserIdentity user={item.user} fallbackId={item.userId} /></td><td><strong className="mono">{item.code}</strong><small className="mono">{item.id}</small></td><td><Badge value={item.status} /></td><td>{item.selfChangesUsed} / 1<small>剩余 {item.selfChangesRemaining} 次</small></td><td>{item.source === 'user' ? '用户修改' : item.source === 'admin' ? '后台重置' : '系统生成'}</td><td>{item.createdAt}</td><td><div className="row-actions wrap">{item.status === 'active' && <button className="button secondary compact" disabled={!can('users.write')} onClick={() => startOperation(item, 'disable')}>停用</button>}{item.status === 'disabled' && <button className="button secondary compact" disabled={!can('users.write')} onClick={() => startOperation(item, 'enable')}>启用</button>}{item.status !== 'retired' && <button className="button secondary compact danger-text" disabled={!can('users.write')} onClick={() => startOperation(item, 'reset')}>重置</button>}</div></td></tr>)}</tbody></table></div><Pagination data={codes.data} onPage={paginate} /></></DataPanel>
      : <DataPanel loading={relations.loading} error={relations.error} retry={relations.reload} empty={!relations.data?.items.length} emptyTitle="没有匹配的邀请关系" emptyDetail="调整邀请人、被邀请人、注册方式或时间后重试。"><><div className="table-wrap"><table><thead><tr><th>邀请人</th><th>被邀请人</th><th>当前绑定代码</th><th>初次绑定方式</th><th>初次绑定 / 最近变更</th></tr></thead><tbody>{relations.data?.items.map((item) => <tr key={`${item.invitee.id}-${item.inviteCodeId}`}><td><UserIdentity user={item.inviter} fallbackId={item.inviter.id} /></td><td><UserIdentity user={item.invitee} fallbackId={item.invitee.id} /></td><td><strong className="mono">{item.inviteCode}</strong><small className="mono">{item.inviteCodeId}</small></td><td>{inviteBindingMethodLabel(item.registrationMethod)}<small>{item.bindingSource === 'admin' ? '后台维护' : '注册绑定'}</small></td><td>{item.createdAt}<small>最近变更 {item.updatedAt}</small></td></tr>)}</tbody></table></div><Pagination data={relations.data} onPage={paginate} /></></DataPanel>}
    <ConfirmDialog open={Boolean(operation)} title={operationTitle} detail={operation?.kind === 'reset' ? `用户将获得新的随机邀请码；${operation.item.code} 会永久失效且无法复用。` : operation?.kind === 'disable' ? '停用后新注册不能再使用该码；重新启用可恢复使用。' : '启用后该码可再次用于新账号注册。'} confirmLabel={operationTitle} danger={operation?.kind !== 'enable'} confirmDisabled={!reason.trim()} onClose={() => { setOperation(undefined); setReason(''); }} onConfirm={executeOperation}><label className="field-label">操作理由<textarea value={reason} maxLength={500} onChange={(event) => setReason(event.target.value)} placeholder="填写工单、用户申请或风控依据" required /></label></ConfirmDialog>
  </>;
}

function GroupsPage({ scope = 'normal' }: { scope?: 'normal' | 'banned' }) {
  const { api, mode, notify, can } = useApi();
  ...
  const [sendGroup,setSendGroup]=useState<GroupRecord>();const [sendContent,setSendContent]=useState('');const [sendReason,setSendReason]=useState('');const [banGroup,setBanGroup]=useState<GroupRecord>();const [banReason,setBanReason]=useState('');
  useEffect(() => {setPage(1);setCursors({1:''});}, [deferredQuery,status]);
  const state = useResource(() => api.getGroups(deferredQuery,status,page,20,cursors[page]??'',scope), [api, mode, deferredQuery,status,page,cursors,scope]);
  ...
  const send=async()=>{if(!sendGroup||!sendContent.trim()||!sendReason.trim())throw new Error('请填写消息与理由');await api.sendGroupMessage(sendGroup.id,sendGroup.ownerId,sendContent.trim(),sendReason.trim());notify('已使用群主身份发送消息');setSendGroup(undefined);await state.reload();};
  const toggleBan=async()=>{if(!banGroup||!banReason.trim())throw new Error('请输入处置理由');await api.setGroupBan(banGroup.id,!banGroup.banned,banReason.trim());notify(banGroup.banned?'群组已解封':'群组已封禁');setBanGroup(undefined);await state.reload();};
  return <><PageHeader title={scope==='banned'?'封禁群列表':'群组列表'} description={scope==='banned'?'仅显示已封禁且未解散的群，可查看历史并恢复通信。':'检查群资料、群主、成员、消息、举报与治理状态。'} /><Toolbar query={query} setQuery={setQuery} placeholder="搜索群名称、群 ID、群主或手机号">{scope==='normal'&&<select className="select-control" aria-label="群组状态" value={status} onChange={e=>setStatus(e.target.value)}><option value="">全部状态</option><option value="active">正常</option><option value="muted">全员禁言</option><option value="dissolved">已解散</option></select>}</Toolbar>
    <DataPanel loading={state.loading} error={state.error} retry={state.reload} empty={!state.data?.items.length} emptyTitle={scope==='banned'?'当前没有封禁群组':'没有匹配的群组'} emptyDetail="请更换群名称、群 ID、群主昵称或手机号后重试。"><div className="table-wrap"><table className="groups-table"><thead><tr><th>群组</th><th>状态</th><th>群主</th><th>成员</th><th>累计消息</th><th>举报</th><th>创建日期</th><th className="table-actions-heading">操作</th></tr></thead><tbody>{state.data?.items.map((group) => <tr key={group.id}><td><div className="group-name"><span>{group.avatarUrl?<img src={group.avatarUrl} alt=""/>:<Users size={17} />}</span><div><strong>{group.name}</strong><small className="mono">{group.id}</small></div></div></td><td><Badge value={group.status} />{group.banReason&&<small>{group.banReason}</small>}</td><td><UserIdentity user={group.owner} fallbackId={group.ownerId}/></td><td>{group.memberCount}</td><td>{group.messageCount.toLocaleString()}</td><td>{group.reportCount}</td><td>{group.createdAt}</td><td className="table-actions-cell"><div className="row-actions group-actions"><button className="row-action" aria-label="查看详情" title="查看群组详情" onClick={() => setDetailGroup(group)}>详情</button>{scope==='normal'&&group.status!=='dissolved'&&<button className="row-action" disabled={!can('groups.write')} onClick={()=>{setSendGroup(group);setSendContent('');setSendReason('');}}>发消息</button>}<button className={`row-action ${group.banned?'':'danger-text'}`} disabled={group.status==='dissolved'||!can('groups.write')} onClick={()=>{setBanGroup(group);setBanReason('');}}>{group.banned?'解封':'封禁'}</button>{scope==='normal'&&<button className="row-action danger-text" aria-label="解散群组" disabled={group.status === 'dissolved' || !can('groups.write')} title={!can('groups.write') ? '当前角色没有群组处置权限' : '不可恢复地解散群组'} onClick={() => openDisband(group)}>解散</button>}</div></td></tr>)}</tbody></table></div><Pagination data={state.data} onPage={paginate} /></DataPanel>
    ...
    <ConfirmDialog open={Boolean(sendGroup)} title="使用群主身份发消息" detail={sendGroup?`发送人：${sendGroup.owner.nickname}（${sendGroup.ownerId}）`:''} confirmLabel="确认发送" confirmDisabled={!sendContent.trim()||!sendReason.trim()} onClose={()=>setSendGroup(undefined)} onConfirm={send}><label className="field-label">文本消息<textarea value={sendContent} maxLength={5000} onChange={event=>setSendContent(event.target.value)} required/></label><label className="field-label">操作理由<textarea value={sendReason} maxLength={500} onChange={event=>setSendReason(event.target.value)} required/></label></ConfirmDialog>
    <ConfirmDialog open={Boolean(banGroup)} title={banGroup?.banned?'解封群组':'封禁群组'} detail={banGroup?.banned?'恢复原有成员的频道通信权限。':'保留群成员与历史消息，但停止频道通信。'} confirmLabel={banGroup?.banned?'确认解封':'确认封禁'} danger={!banGroup?.banned} confirmDisabled={!banReason.trim()} onClose={()=>setBanGroup(undefined)} onConfirm={toggleBan}><label className="field-label">操作理由<textarea value={banReason} maxLength={500} onChange={event=>setBanReason(event.target.value)} required/></label></ConfirmDialog>
  </>;
}
...
function messageLifecycle(message: MessageRecord) {
  if(message.deletedForEveryoneAt) return {value:'rejected',label:'已全端删除',detail:`${dateTimeLabel(message.deletedForEveryoneAt)} · ${message.deletedForEveryoneBy || '未知'}`};
...
const administratorUsernamePattern = /^[a-z0-9][a-z0-9._-]{1,30}[a-z0-9]$/;
type AdministratorEditor = { mode: 'create' | 'edit'; account?: AdministratorRecord; username: string; email: string; displayName: string; roleId: string; password: string; reason: string };
...
function AdministratorsPage() {
  const { api, mode, notify, session, logout } = useApi();
  ...
  const openCreate = () => setEditor({ mode: 'create', username: '', email: '', displayName: '', roleId: roles.data?.find((role) => role.id === 'support')?.id ?? roles.data?.[0]?.id ?? '', password: '', reason: '' });
  const openEdit = (account: AdministratorRecord) => setEditor({ mode: 'edit', account, username: account.username, email: account.email ?? '', displayName: account.displayName, roleId: account.roleId, password: '', reason: '' });
  const saveAccount = async () => {
    if (!editor) return;
    const username = editor.username.trim().toLowerCase();
    const email = editor.email.trim().toLowerCase();
    if (editor.mode === 'create') await api.createAdministrator({ username, ...(email ? { email } : {}), displayName: editor.displayName.trim(), roleId: editor.roleId, password: editor.password }, editor.reason.trim());
    else if (editor.account) {
      const selfRenamed = editor.account.id === session.id && username !== session.username;
      await api.updateAdministrator(editor.account.id, { username, email: email || null, displayName: editor.displayName.trim(), roleId: editor.roleId }, editor.reason.trim());
      if (selfRenamed) {
        notify('管理员账号已更新，请使用新账号重新登录');
        logout();
        return;
      }
    }
  ...
  const normalizedEditorUsername = editor?.username.trim().toLowerCase() ?? '';
  const editorEmail = editor?.email.trim() ?? '';
  const editorEmailValid = !editorEmail || /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(editorEmail);
  const editorValid = Boolean(editor && administratorUsernamePattern.test(normalizedEditorUsername) && editorEmailValid && editor.displayName.trim() && editor.roleId && editor.reason.trim() && (editor.mode === 'edit' || (editor.password.length >= 8 && editor.password.length <= 128)));
...
function SettingsPage() {
  ...
  return <><PageHeader title="系统设置" description="统一管理可热更新的业务策略；敏感密钥只展示配置状态，基础设施参数需修改环境变量并重启服务。" />{state.loading ? <Skeleton rows={8} /> : state.error || !form ? <ErrorState message={state.error} retry={state.reload} /> : <form className="settings-layout" onSubmit={(event) => void submit(event)}>
    <div className="settings-main"><section className="settings-section"><div className="settings-title"><CircleUserRound size={20} /><div><h2>注册与登录</h2><p>手机号验证码始终用于注册、换绑和找回密码；关闭注册不会影响已有账号登录。</p></div></div><Toggle label="允许新用户注册" description="关闭后密码注册和验证码首次开户都会被拒绝。" checked={form.allowRegistration} onChange={(value) => change('allowRegistration', value)} /><div className="form-grid"><label className="field-label">邀请码策略<select value={form.inviteRegistrationMode} onChange={(event) => change('inviteRegistrationMode', event.target.value as AdminSettings['inviteRegistrationMode'])}><option value="disabled">关闭邀请码功能</option><option value="optional">邀请码选填（默认）</option><option value="required">新账号必须填写</option></select><small>已有账号验证码登录、后台单个和批量开户不受影响。</small></label><label className="field-label">密码最少字符数<input type="number" min="8" max="16" value={form.passwordMinLength} onChange={(event) => change('passwordMinLength', Number(event.target.value))} required /></label></div></section>
      <section className="settings-section"><div className="settings-title"><MessageSquareText size={20} /><div><h2>消息、撤回与文件</h2><p>私聊和群聊撤回分别设置，默认均为 24 小时，最长 7 天；消息编辑使用独立时限。群主、管理员及普通成员统一受群聊撤回时限约束。上传上限由基础设施参数控制。</p></div></div><div className="form-grid"><label className="field-label">文本最大字数<input type="number" min="100" max="10000" value={form.maxMessageTextLength} onChange={(event) => change('maxMessageTextLength', Number(event.target.value))} required /></label><label className="field-label">私聊撤回时限（分钟）<input type="number" min="1" max="10080" step="1" value={form.directRecallMinutes} onChange={(event) => change('directRecallMinutes', Number(event.target.value))} required /></label><label className="field-label">群聊撤回时限（分钟）<input type="number" min="1" max="10080" step="1" value={form.groupRecallMinutes} onChange={(event) => change('groupRecallMinutes', Number(event.target.value))} required /></label><label className="field-label">消息编辑时限（分钟）<input type="number" min="1" max="1440" step="1" value={form.messageRecallMinutes} onChange={(event) => change('messageRecallMinutes', Number(event.target.value))} required /></label></div></section>
...
function Toggle({ label, description, checked, disabled = false, onChange }: { label: string; description: string; checked: boolean; disabled?: boolean; onChange: (value: boolean) => void }) {
  return <label className={`toggle-row${disabled ? ' disabled' : ''}`}><div><strong>{label}</strong><span>{description}</span></div><input type="checkbox" aria-label={label} checked={checked} disabled={disabled} onChange={(event) => onChange(event.target.checked)} /><span className="toggle-control" aria-hidden="true" /></label>;
}
...
function DataPanel({ loading, error, retry, empty, emptyTitle, emptyDetail, emptyIcon, children }: { loading: boolean; error: string; retry: () => void; empty: boolean; emptyTitle: string; emptyDetail: string; emptyIcon?: ReactNode; children: ReactNode }) {
  return <div className="data-panel" aria-busy={loading}>{loading ? <Skeleton rows={6} /> : error ? <ErrorState message={error} retry={retry} /> : empty ? <EmptyState title={emptyTitle} detail={emptyDetail} icon={emptyIcon ?? contextualEmptyIcon(emptyTitle)} /> : children}</div>;
}

function LoginPage({ onLogin, sessionNotice = '' }: { onLogin: (username: string, password: string) => Promise<void>; sessionNotice?: string }) {
  const [username, setUsername] = useState(''), [usernameDirty, setUsernameDirty] = useState(false), [usernameTouched, setUsernameTouched] = useState(false), [password, setPassword] = useState(''), [error, setError] = useState(''), [submitting, setSubmitting] = useState(false), [showPassword, setShowPassword] = useState(false);
  const normalizedUsername = username.trim().toLowerCase();
  const usernameInvalid = normalizedUsername.length > 0 && !administratorUsernamePattern.test(normalizedUsername);
  const usernameError = usernameTouched ? (!normalizedUsername ? '请输入管理员账号' : usernameInvalid ? '请输入有效的管理员账号' : '') : '';
  ...
  return <main className="login-screen">
    <section className="login-form-panel" aria-labelledby="login-title"><div className="login-card">
      ...
      <form onSubmit={(event) => void submit(event)} noValidate>
        <div className="field-label"><label htmlFor="admin-username">管理员账号</label><input id="admin-username" type="text" value={username} minLength={3} maxLength={32} autoCapitalize="none" spellCheck={false} onBlur={() => { if (usernameDirty) setUsernameTouched(true); }} onChange={(event) => { setUsername(event.target.value); setUsernameDirty(true); if (error) setError(''); }} autoComplete="username" required autoFocus aria-invalid={Boolean(usernameError)} aria-describedby="admin-username-help" /><small id="admin-username-help" className={usernameError ? 'field-error' : 'field-hint'}>{usernameError || '使用管理员分配的登录账号'}</small></div>
        <div className="field-label"><label htmlFor="admin-password">密码</label><div className="login-password-input"><input id="admin-password" aria-label="密码" type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => { setPassword(event.target.value); if (error) setError(''); }} autoComplete="current-password" required /><button className="login-password-toggle" type="button" aria-label={showPassword ? '隐藏密码' : '显示密码'} aria-pressed={showPassword} onClick={() => setShowPassword((visible) => !visible)}>{showPassword ? <EyeOff size={17} /> : <Eye size={17} />}</button></div></div>
        {error && <div className="inline-notice danger" role="alert"><AlertTriangle size={15} />{error}</div>}
...
function readSession(): { session?: AdminSession; notice?: string } {
  ...
  try {
    ...
    if (value?.token && value.expiresAt > Date.now() && value.id && value.username && value.roleId && Array.isArray(value.permissions)) {
    ...
    sessionStorage.removeItem(LEGACY_SESSION_KEY);
    return { notice: value?.token && typeof value.expiresAt === 'number' && value.expiresAt <= Date.now() ? '管理员会话已到期，请重新登录' : '登录信息无效，请重新登录' };
...
export function App() {
  ...
  const login = async (username: string, password: string) => { const candidate = await loginAdmin(username, password); sessionStorage.setItem(SESSION_KEY, JSON.stringify(candidate)); sessionStorage.removeItem(LEGACY_SESSION_KEY); setSessionNotice(''); setSession(candidate); };
