...
function UserManagementCard({ user, canWrite, onDetail, onRecords, onMessage, onDisposition,onIP }: { user: UserRecord; canWrite: boolean; onDetail: () => void; onRecords: () => void; onMessage: () => void; onDisposition: () => void;onIP:(ip:string)=>void }) {
  const {notify}=useApi();
...
function UsersPage({ scope = 'all' }: { scope?: 'all' | 'banned' }) {
  ...
  useEffect(() => { setPage(1); setCursors({ 1: '' }); }, [deferredQuery, status, pageSize,ip,ipSource]);
  const state = useResource(() => api.getUsers(deferredQuery, status, page, pageSize, cursors[page] ?? '',ip,ipSource), [api, mode, deferredQuery, status, page, pageSize, cursors,ip,ipSource]);
