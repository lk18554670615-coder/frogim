...
import type {
  ...
  LiveKitMetrics, LiveKitParticipant, LiveKitRoom, WukongChannel, WukongConnection, WukongDevice, WukongNode, WukongPlugin, WukongPluginEvent, WukongPluginLogEntry, WukongStoredMessage, WukongSystemUser,
...
function OperationsPage(){
  ...
  if(state.loading)return <><PageHeader title="推送、备份、任务与权限" description="只读运行视图。"/><Skeleton rows={7}/></>;
  if(state.error||!state.data)return <><PageHeader title="推送、备份、任务与权限" description="只读运行视图。"/><ErrorState message={state.error} retry={state.reload}/></>;
  ...
  return <><PageHeader title="推送、备份、任务与权限" description="查看推送设备和队列、备份结果、后台任务积压、当前管理员与角色边界；页面不提供命令执行、恢复或手动清理。" actions={<button className="button secondary" onClick={()=>void state.reload()}><RefreshCcw size={15}/>刷新</button>}/>
