...
describe('live API adapter', () => {
  ...
  it('运行概览缺少计数时显示未上报而不是伪造成零', async () => {
    ...
    expect(dashboard.metrics).toEqual([
      expect.objectContaining({ label: '用户总数', value: '—', delta: '封禁账号数未上报', tone: 'warning' }),
      expect.objectContaining({ label: '累计消息', value: '—', delta: '消息总数未上报', tone: 'warning' }),
      expect.objectContaining({ label: '待审举报', value: '—', delta: '举报队列未上报', tone: 'warning' }),
      expect.objectContaining({ label: 'WuKong 连接', value: '—', delta: '业务会话数未上报', tone: 'warning' }),
    ]);
  });
