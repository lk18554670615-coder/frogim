...
function adaptUser(value: unknown): UserRecord {
  ...
  return {
		isInternalUser: boolean(raw.isInternalUser),
...
function adaptInternalUserUpdate(value: unknown): InternalUserUpdate {
  const raw = object(value);
  return {
...
function adaptDashboard(payload: unknown): DashboardData {
  ...
  if (Array.isArray(raw.metrics)) {
    return {
      ...
      activity: list(raw.activity).map(adaptAudit), generatedAt: string(raw.generatedAt) ? formatDate(raw.generatedAt) : undefined,
      trendWindowHours: number(raw.trendWindowHours, 24), mixWindowHours: number(raw.mixWindowHours, 24),
    };
  }
  const users = optionalNumber(raw.users);
  const newUsersToday = optionalNumber(raw.newUsersToday);
  const bannedUsers = optionalNumber(raw.bannedUsers);
  const groups = optionalNumber(raw.groups);
  const newGroupsToday = optionalNumber(raw.newGroupsToday);
  const messages = optionalNumber(raw.messages);
  const messagesToday = optionalNumber(raw.messagesToday);
  const reports = optionalNumber(raw.pendingReports);
  const conversations = optionalNumber(raw.conversations);
  const generatedAt = string(raw.generatedAt) ? formatDate(raw.generatedAt) : undefined;
  ...
  if (!alerts.length && reports !== null && reports > 0) alerts.push({
    id: 'pending-reports', title: '存在待审举报', detail: `${reports} 条举报等待处理`, severity: reports >= 10 ? 'critical' : 'warning', time: generatedAt ?? '实时',
  });
  if (raw.realtimeStatus === 'unavailable' || raw.realtimeStatus === 'degraded' || raw.wukongStatus === 'unavailable') alerts.push({
    id: 'realtime-unavailable', title: '即时通信服务状态异常', detail: '请检查通讯节点连接和服务健康状态', severity: 'critical', time: generatedAt ?? '实时',
  });
  return {
    metrics: [
      { label: '用户总数', value: users?.toLocaleString() ?? '—', delta: newUsersToday !== null ? `今日新增 ${newUsersToday}` : bannedUsers === null ? '用户统计未完整上报' : `${bannedUsers} 个封禁账号`, tone: users === null ? 'warning' : 'info' },
      { label: '活跃群组', value: groups?.toLocaleString() ?? '—', delta: newGroupsToday !== null ? `今日新建 ${newGroupsToday}` : conversations === null ? '群组统计未上报' : `${conversations} 个业务会话`, tone: groups === null && conversations === null ? 'warning' : 'info' },
      { label: '今日消息', value: messagesToday?.toLocaleString() ?? messages?.toLocaleString() ?? '—', delta: messages === null ? '消息统计未上报' : messagesToday === null ? '当前仅提供累计消息' : `累计 ${messages.toLocaleString()}`, tone: messages === null ? 'warning' : 'success' },
    ...
    messageTrend, channelMix, alerts, activity: list(raw.activity).map(adaptAudit), generatedAt,
    trendWindowHours: number(raw.trendWindowHours, 24), mixWindowHours: number(raw.mixWindowHours, 24),
  };
}
...
function liveApi(token: string): AdminApi {
  return {
    ...
    async setUserInternalStatus(id, internal, reason) { return adaptInternalUserUpdate(await request(`/users/${encodeURIComponent(id)}/internal-user`, token, {method:'PUT', body:JSON.stringify({isInternalUser:internal,reason,confirmed:true})})); },
    ...
    async setGroupHistoryVisibility(id, visible, reason) { await request(`/groups/${encodeURIComponent(id)}/history-visibility`, token, { method: 'PUT', body: JSON.stringify({ historyVisibleToNewMembers: visible, reason, confirmed: true }) }); },
