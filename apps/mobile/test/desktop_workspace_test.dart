...
void main() {
  ...
  testWidgets('桌面工作台在宽屏居中并保留外层留白', (tester) async {
    const viewport = Size(1600, 1000);
    final controller = await _pumpDesktopHome(tester, size: viewport);
    addTearDown(controller.dispose);

    final frame = tester.getRect(find.byKey(const Key('desktop-home-frame')));
    expect(frame.width, 1360);
    expect(frame.height, 860);
    expect(frame.left, (viewport.width - frame.width) / 2);
    expect(frame.top, (viewport.height - frame.height) / 2);
    expect(find.byKey(const Key('desktop-home-shell')), findsOneWidget);
    expect(tester.takeException(), isNull);
  });
