...
- PostgreSQL 业务数据：聊天消息、会话序号、同步游标、通话状态和推送 Outbox，是业务真相源。
...
| `server` | IM 业务与通讯节点 | 登录、消息事务、WebSocket、推送、群聊、通话信令 |
| `admin` | 运营管理后台 | 静态资源、后台页面健康 |
| `postgres` | 主数据库 | 启动恢复、连接、事务、WAL、慢查询 |
| `redis` | 集群临时状态 | 在线事件、临时信令、全局限流、AOF |
| `minio` | 私有媒体存储 | 头像、图片、语音、视频、文件和磁盘容量 |
| `coturn` | 音视频中继 | STUN/TURN 鉴权、分配、端口和会话异常 |
...
严禁写入消息正文、手机号验证码、访问/刷新令牌、TOTP、推送 CID、数据库密码、MinIO 密钥和 TURN 凭据。需要业务追溯时使用管理后台“审计日志”，不能通过扩大运行日志内容解决。
